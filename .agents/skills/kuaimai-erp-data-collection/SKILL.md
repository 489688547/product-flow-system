...
用已登录 Chrome 生成快麦官方 XLS/XLSX/CSV 导出文件，再通过项目采集器预检、分块并幂等写入 D1。当前商品采集不依赖未打通的快麦开放 API，不得把待验证描述为已接通。
...
快麦订单页的“导出订单/导出订单明细”是异步任务，不会直接产生 Chrome 下载。自动采集必须在点击导出前记录任务开始时间，再进入固定下载中心；订单只匹配 `快麦ERP交易订单导出`，订单明细只匹配 `快麦ERP交易订单明细导出`，并仅选择本次任务窗口内的最近记录。“导出完成”后点击同行官方下载；“生成中”继续轮询；平台生成失败或三分钟仍未出现匹配文件时，停止当前任务并记录对应安全错误码，不能用历史文件或其他 XLSX 冒充本次结果。

商品刷新必须一次创建并完成三个当前快照任务，不能只导普通商品：

| 资源 | 快麦导出项 | 下载中心文件名前缀 |
|---|---|---|
| `products` | 导出普通商品 | `快麦导出_普通商品` |
| `product_kits` | 导出套件 | `快麦导出_套件` |
| `product_combinations` | 导出组合装 | `快麦导出_组合装` |

固定页面为 `#/prod/parallel/`。每个任务依次选择导出项、XLS 格式、确认官方字段选择、确认生成文件，再进入下载中心匹配本任务窗口内的最近文件。套件和组合装必须保留官方组件关系，至少识别 `子商品商家编码` 和 `组合比例`；不得根据商品名、编码前缀或长度猜组合关系。三个任务全部成功并完成 D1 投影后才刷新商品目录；任一失败都保留上一次完整目录，并在同步记录中显示具体资源和恢复动作。人工导入同三类官方文件仅作为 Chrome 采集失败时的后备路径。
...
- `KUAIMAI_EXPORT_GENERATION_FAILED`：平台已明确生成失败；缩短日期范围后重新创建导出，不能把历史文件标记为本次完成。
- `KUAIMAI_DOWNLOAD_CENTER_TIMEOUT`：下载中心三分钟内没有本次资源的匹配记录；先确认平台是否仍在生成，再缩短日期范围重试。
- `EXTENSION_DOWNLOAD_TIMEOUT`：已经点击下载中心文件但 Chrome 下载未完成；检查 Chrome 下载状态和磁盘，不要重复使用其他文件。
- 商品字段选择或表头变化：停止对应快照；保存脱敏表头证据并补映射测试。不能跳过套件或组合装后把普通商品结果标记为完整刷新。
