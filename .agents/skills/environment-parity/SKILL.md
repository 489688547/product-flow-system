...
description: Use when work in product-flow-system may add or change environment variables, database bindings or tables, production-data access, Aliyun runtimes, Cloudflare static-test deployment, or external-provider configuration.
---
...
3. Classify every dependency as variable name, secret, binding, SQLite table, production-data access, static frontend, or external-provider action.
...
5. Choose the documented path. Operational repair through the production-data gateway requires a hashed token, active executive identity, 15-minute unlock, version check, write-before snapshot, audit, and rollback. `npm start` is local SQLite sandbox only. Shared acceptance uses `https://test.deshan-tiyes.cn` with `https://api-test.deshan-tiyes.cn`; production uses `https://deshan-tiyes.cn`. Never add remote D1, a local production impersonation transport, or browser-held secrets.
6. Treat production database writes and real DingTalk, Kuaimai, Aliyun, or other provider actions as separate execution boundaries. 生产数据库写入权不等于外部平台真实操作权。Only a verified deployed session entering the existing route authorization and provider adapter may exercise both boundaries; localhost or database access alone grants nothing.
...
After deployment, run `npm run verify:production -- --require-platform <platform-id>` for every routed or `Integration-Impact` platform with the ignored operational token. Check local sandbox, fixed test frontend, ECS test API, ECS production, and provider runtime separately; never infer one from another. Any affected-platform warning blocks completion, handoff, deployment-success, and readiness claims.
...
| “测试站已通过” | ECS production still needs its own readiness and runtime verification. |
| “本地就当线上账号” | Local is an isolated SQLite sandbox; use the fixed test or production domain for real account verification. |
...
- Direct production database write path from the browser.
- Hard-coded local employee identity, remote database access, or a production-account transport in the supported `npm start` runtime.
