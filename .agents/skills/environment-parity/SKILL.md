...
5. Reuse the server-side production-data gateway. Local production reads must use it. Writes require a hashed token, active executive identity, 15-minute unlock, version check, write-before snapshot, audit, and rollback. Never expose tokens to the browser or add a direct D1 bypass.
6. Treat production database writes and real DingTalk, Kuaimai, Aliyun, or other provider actions as separate authorities. 生产数据库写入权不等于外部平台真实操作权。
...
| “有数据库最高权限” | It grants neither provider actions nor GitHub/Cloudflare/Aliyun control-plane authority. |
