...
const environmentCapabilities = {
  ...
  "capabilities": [
    ...
    },
    {
      ...
      "name": "公司网页数据采集运行时",
      "description": "公司 Mac 共用本机 loopback、固定 provider adapter、日计划、任务租约和安全检查点，销售异常可按店铺与业务日期幂等排队：快麦继续通过 Chrome MV3 采集官方文件，抖店只通过按稳定店铺 ID 隔离的 Ego Task Space 采集。抖店结果在阿里云 ECS API 的 SQLite 事务确认前只保留本地待上传证据，Cloudflare D1 不接收 Ego 写入；实验模板仍需服务端与公司 Mac 双开关并只产生 untrusted 或 validated 结果。",
      "platforms": [
        "cloudflare-pages",
        "cloudflare-d1",
        "aliyun",
