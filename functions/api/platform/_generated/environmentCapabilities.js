...
const environmentCapabilities = {
  ...
  "capabilities": [
    ...
    {
      "id": "aliyun-ecs-runtime",
      "name": "阿里云 ECS 过渡生产运行时",
      "description": "ECS 容器通过 Wrangler 本地 Pages 运行时复用现有 Functions，并把正式与展示 D1 binding 持久化为数据卷内两个独立 SQLite；该能力由阿里云专用预检阻断，不改变 Cloudflare Production 的全局 readiness 要求。",
      "platforms": [
        "aliyun",
        "cloudflare-pages",
        "cloudflare-d1",
    ...
    },
    {
      ...
      "name": "阿里云 OSS 私有备份",
      "description": "ECS 对两个本地 D1 生成一致性 SQL 导出和 SHA-256 清单，并在私有 Bucket 与实例 RAM 角色就绪后上传 OSS；仓库只登记目标名称和地域，不登记 AccessKey。",
      "platforms": [
        "aliyun",
        "cloudflare-d1"
      ],
    ...
    },
    {
      ...
      "name": "本地线上真实账号",
      "description": "本地 Pages Functions 构建为远程开发 Worker，通过每次启动的一次性服务端密钥和个人令牌还原真实最高权限身份，并原生访问生产 D1 与外部平台路由。",
      "platforms": [
        "cloudflare-pages",
        "cloudflare-d1",
        "dingtalk",
    ...
    },
    {
      ...
      "name": "抖店罗盘 Ego 采集",
      "description": "账号密码登录保持退役；公司 Mac 通过按稳定店铺 ID 隔离的 Ego Task Space 下载官方报表。本机完成归档、解析和待上传检查点，正式事实只由阿里云 ECS API 写入 SQLite；Cloudflare D1 仅保留回滚边界且不接收 Ego 写入。ICP备案与单任务验收完成前不声明全局生产就绪。",
      "platforms": [
        "douyin-ecommerce",
        "erp-file-import",
        "aliyun",
    ...
    },
    {
      ...
      "name": "快麦 ERP 文件采集",
      "description": "通过快麦后台官方导出文件补录订单、商品、库存等 ERP 数据；原始文件保存在公司 Mac，D1 按内容哈希保存归档清单、最小索引、批次与异常审计。归档运行状态与显式入库决策分开保存，处理超过 24 小时自动转失败，有意跳过必须记录受控原因。",
      "platforms": [
        "kuaimai",
        "erp-file-import",
        "cloudflare-pages",
    ...
    },
    {
      ...
      "name": "公司网页数据采集运行时",
      "description": "公司 Mac 共用本机 loopback、固定 provider adapter、日计划、任务租约和安全检查点，销售异常可按店铺与业务日期幂等排队：快麦继续通过 Chrome MV3 采集官方文件，抖店只通过按稳定店铺 ID 隔离的 Ego Task Space 采集。抖店结果在阿里云 ECS API 的 SQLite 事务确认前只保留本地待上传证据，Cloudflare D1 不接收 Ego 写入；实验模板仍需服务端与公司 Mac 双开关显式启用，并只产生 untrusted 或 validated 结果。",
      "platforms": [
        "cloudflare-pages",
        "cloudflare-d1",
        "aliyun",
