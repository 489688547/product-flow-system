...
const integrationRegistry = {
  ...
  "platforms": [
    ...
    },
    {
      ...
      "name": "快麦开放平台",
      "status": "integrating",
      "summary": "快麦开放平台 API 当前未打通且产品不调用；公司 Mac 使用 Chrome 插件驱动快麦后台官方导出，官方文件负责完整销售、商品和历史补数，原始文件留在本机，D1 保存通用任务、归档清单和业务投影。",
      "capabilities": [
        "官方文件历史补数",
        "公司 Mac 每日网页采集",
        "订单创建时间口径",
      ...
      "relations": [
        {
          ...
          "description": "快麦任务、游标、归档清单、批次、共享商品目录、组合关系、最小索引和异常审计写入 D1；原始文件留在公司 Mac。"
        },
        {
    ...
    },
    {
      ...
      "apiRoutes": [
        ...
        "/api/platform/v1/data-services/sales/daily",
        "/api/platform/v1/data-services/sales-repair",
        "/api/platform/v1/data-tasks",
        "/api/platform/v1/goods-flow/",
        "/api/platform/v1/supply-chain-workflows/",
        "/api/platform/v1/web-collection/jobs",
        "/api/platform/v1/web-collection/runners",
    ...
    },
    {
      ...
      "name": "Cloudflare D1",
      "status": "connected",
      "summary": "保存共享业务状态、平台数据、登录会话、组织数据、研发待办、销售聚合、共享商品目录、用户洞察、货流事实、供应链版本化工作流、数据中心、加密凭证、店铺运营、人事绩效、跨 App 协同和 AI 安全元数据。",
      "capabilities": [
        "共享状态持久化",
        "共享状态原子比较并写入",
        "共享状态写前快照与审计",
      ...
      "codePaths": [
        ...
        "functions/api/platform/v1/goods-flow/**",
        "functions/api/platform/v1/data-services/**",
        "functions/api/platform/v1/data-tasks.js",
        "functions/api/platform/v1/supply-chain-workflows/**",
        "functions/api/sales.js",
        ...
        "migrations/0016_supply_chain_workflows.sql"
      ],
      "envVars": [
        "PRODUCT_FLOW_DB",
        "DEMO_FLOW_DB"
    ...
    },
    {
      ...
      "name": "阿里云",
      "status": "integrating",
      "summary": "ECS 作为 Pages Functions 与双 SQLite 的过渡生产运行时，OSS 保存私有图片、附件和数据库备份；Cloudflare 在正式切流验收前保持回滚边界。",
      "capabilities": [
        "云资源",
        "ECS 容器运行时",
        "双 SQLite 持久化",
      ...
      "relations": [
        {
          "platformId": "cloudflare-pages",
          "type": "migration-and-rollback-boundary",
          "description": "ECS 私有预发布与最终切流独立验收；Cloudflare 在切流前继续生产服务并保留为明确回滚入口。"
        },
        {
          "platformId": "cloudflare-d1",
          "type": "migration-source",
          "description": "正式与展示 D1 通过停写窗口全量导出到 ECS 两个独立本地 SQLite，不进行长期双写。"
        },
        {
          "platformId": "dingtalk",
          "type": "hosts-callbacks",
