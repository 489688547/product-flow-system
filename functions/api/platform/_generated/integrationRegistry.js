  ...
  "platforms": [
    {
      ...
      "relations": [
        {
          "platformId": "aliyun",
          "type": "hosts-callbacks-and-data",
          "description": "阿里云 ECS 承载登录回调和开放平台 API，SQLite 保存登录会话、用户令牌与组织同步结果。"
        }
      ]
    },
    {
      ...
      "name": "快麦开放平台",
      "status": "integrating",
      "summary": "快麦开放平台 API 当前未打通且产品不调用；公司 Mac 使用 Chrome 插件驱动快麦后台官方导出，官方文件负责完整销售、商品和历史补数，原始文件留在本机，阿里云 SQLite 保存通用任务、归档清单和业务投影。",
      "capabilities": [
        "官方文件历史补数",
        "公司 Mac 每日网页采集",
        "订单创建时间口径",
      ...
      "codePaths": [
        ...
        "functions/api/platform/v1/data-services/_shared/salesRepairResolution.js",
        ...
        "src/domain/webCollection.js",
        "src/domain/dataSyncRecovery.js",
        ...
        "src/state/webCollectionApi.js",
        "src/features/data-center/DataCenterAppPage.jsx",
        ...
        "migrations/0009_web_collection.sql",
        "docs/features/kuaimai-erp-history/**",
        "docs/features/kuaimai-erp-local-archive/**",
        "docs/features/company-web-data-collector/**",
        "docs/features/data-warning-recovery/**",
        "server.mjs"
      ],
      "envVars": [
        "KUAIMAI_APP_KEY",
      ...
      "evidence": [
        ...
        "docs/platform/apis/web-collection-v1.md",
        "docs/features/data-warning-recovery/prd.md",
      ...
      "relations": [
        {
          ...
          "description": "快麦任务、游标、归档清单、批次、共享商品目录、组合关系、最小索引和异常审计写入 ECS SQLite；原始文件留在公司 Mac。"
        },
        {
          "platformId": "erp-file-import",
          "type": "primary-current-channel",
          "description": "Chrome 插件驱动的官方导出与人工官方文件是当前快麦历史、日常和完整口径的主通道；开放平台 API 暂不使用。"
        }
      ]
    },
    {
      ...
      "name": "抖音电商罗盘",
      "status": "integrating",
      "summary": "账号密码登录与旧 browser-agent 任务保持退役；抖店正式采集只使用公司 Mac 的 Ego Task Space 复用人工维护的登录态，按稳定店铺 ID 隔离多店铺任务并下载官方报表。本机完成受控下载、归档、解析和检查点；只有阿里云 ECS API 的 SQLite 事务确认后才成功。",
      "capabilities": [
        "多店铺名称与店铺 ID 目录",
      ...
      "apiRoutes": [
        "/api/platform/v1/web-collection/jobs",
        "/api/platform/v1/web-collection/runners",
        "/api/platform/v1/commerce-facts",
        "/api/platform/v1/commerce-facts/ingest"
      ],
      "publicDocs": [],
      "evidence": [
      ...
      "relations": [
        ...
        {
          "platformId": "erp-file-import",
          "type": "manual-fallback-and-file-runtime",
          "description": "官方报表解析、原始归档和文件导入复用共同的安全文件原则，人工导入保留为兜底。"
        },
        {
          "platformId": "kuaimai",
    ...
    },
    {
      ...
      "status": "connected",
      "summary": "仅托管 dev 分支测试静态前端；不运行 Pages Functions、Workers、D1 或业务 Secret。浏览器直接调用阿里云 ECS 的隔离测试 API。",
      "capabilities": [
        "测试静态前端"
      ],
      "businessQuestions": [
        "测试前端构建或静态部署失败",
        "测试前端 commit 与 ECS 测试 API 不一致",
        "测试自定义域名不可用"
      ],
      "keywords": [
        "Cloudflare",
      ...
      "codePaths": [
        ".github/workflows/deploy-test-static.yml",
        "scripts/prepare-runtime-build.mjs",
        "_headers",
        "_redirects"
      ],
      "envVars": [],
      ...
      "evidence": [
        "docs/superpowers/specs/2026-08-06-aliyun-production-static-test-design.md",
        ".github/workflows/deploy-test-static.yml",
        "scripts/prepare-runtime-build.mjs",
        "_headers",
        "_redirects"
      ...
      "relations": [
        {
          "platformId": "aliyun",
          "type": "static-front-to-test-api",
          "description": "测试静态前端从浏览器直接调用 api-test.deshan-tiyes.cn 上的隔离测试 API。"
        }
      ]
    },
    {
      ...
      "name": "Cloudflare D1",
      "status": "retired",
      "summary": "业务数据已迁移到阿里云 ECS SQLite；运行时不再读取、写入或绑定 D1。该条目只保留历史迁移证据。",
      "capabilities": [
        "历史迁移来源"
      ],
      "businessQuestions": [
      ...
      "relations": [
        {
          "platformId": "aliyun",
          "type": "retired-migration-source",
          "description": "正式与展示 D1 已一次性导入两个独立 SQLite；不双写、不保留运行时回退。"
        }
      ]
    },
    {
      ...
      "name": "阿里云",
      "status": "connected",
      "summary": "ECS 承载生产前端、生产 API、隔离测试 API 和两个 SQLite；OSS 保存私有图片、附件和数据库备份。测试静态前端单独托管在 Cloudflare Pages。",
      "capabilities": [
        "生产前端与 API",
        "测试 API",
        "ECS 容器运行时",
      ...
      "relations": [
        {
          ...
          "type": "hosts-static-test-front",
          "description": "Cloudflare Pages 只托管测试静态前端，浏览器直接调用 ECS 测试 API。"
        },
        {
          "platformId": "cloudflare-d1",
          "type": "retired-migration-source",
          "description": "正式与展示 D1 已一次性导入两个独立本地 SQLite；运行时不双写、不回退 D1。"
        },
        {
          "platformId": "dingtalk",
          "type": "hosts-callbacks",
          "description": "域名实名、ICP备案、HTTPS 和钉钉回调白名单完成后，ECS 承载同源登录 callback。"
        }
      ]
    },
    {
      ...
      "name": "ERP / 文件导入",
      "status": "integrating",
      "summary": "承接销售明细、ERP 商品档案、库存单位编码、组合关系、库存快照和月度盘点文件；快麦原始文件在公司 Mac 哈希归档，线上只接收脱敏最小索引和共享业务投影，作为 API 同步的校准、补齐和兼容通道。",
      "capabilities": [
        "快麦官方文件历史补数",
        "快麦每日网页导出",
        "本地原始归档",
      ...
      "relations": [
        {
          "platformId": "kuaimai",
          "type": "primary-history-channel-for",
          "description": "官方导出文件承接快麦开放平台无法完整覆盖的历史数据。"
        },
        {
          "platformId": "aliyun",
          "type": "stores-imports",
          "description": "导入归档清单、批次、最小索引、异常、聚合销售结果和标准化商品目录通过内部 API 写入 ECS SQLite；完整原文件不进入 SQLite。"
        }
      ]
    },
    {
    ...
    },
    {
      ...
      "relations": [
        {
          "platformId": "aliyun",
          "type": "writes-through-and-stores-standard-facts",
          "description": "采集器只能通过受限 ECS API ingest 契约写入 SQLite；SQLite 只保存标准事实、规则、质量和审计，不保存完整页面、Cookie 或凭证。"
        }
      ]
    },
    {
      ...
      "relations": [
        {
          "platformId": "aliyun",
          "type": "called-and-governed-by",
          "description": "ECS API 在服务端调用 Provider，浏览器不接触密钥或公司上下文；SQLite 保存 Provider 安全元数据、外发策略、并发租约和无内容审计。"
        }
      ]
    },
    {
