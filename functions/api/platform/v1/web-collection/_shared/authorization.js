...
const TEMPLATE_EDITOR_DEPARTMENTS = new Set(["总经办", "数据中心", "数据部"]);
...
export function authorizeCollectorTemplateWrite(session) {
  const actor = authorizeWebCollectionView(session);
  const departments = String(session?.department || session?.departmentName || "")
    .split(/[\/,，、]/)
    .map(value => value.trim())
  ...
  if (
    ...
    || (
      ...
      && session?.role !== "data_admin"
      && !departments.some(department => TEMPLATE_EDITOR_DEPARTMENTS.has(department))
    )
  ) {
    throw routeError(403, "COLLECTOR_TEMPLATE_ACTION_DENIED", "当前账号不能修改采集模板。");
  }
  return {
    ...actor,
    userId: String(session.userId || session.id || "").slice(0, 160),
    role: session.role === "executive" ? "executive" : "data_admin"
  };
}
