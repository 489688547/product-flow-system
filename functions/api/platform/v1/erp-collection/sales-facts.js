import { normalizeErpSalesFactsPayload } from "../../../../../src/domain/kuaimaiErpCollection.js";
import {
...
import {
  ...
  ingestProjectedSalesCollection
...
export async function onRequest({ request, env, data = {} }) {
  ...
  try {
    ...
    const normalized = normalizeErpSalesFactsPayload(body, { idempotencyKey });
    ...
    const businessDb = data.businessDb || await resolveCollectionBusinessDatabase({ env, controlDb, target });
    const result = await ingestProjectedSalesCollection(controlDb, normalized, {
      ...actor,
