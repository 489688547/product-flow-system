...
浅色、克制的产品工作台，参考 Linear、Apple 系统应用和成熟钉钉工作台应用。整体采用低饱和暖色系：暖纸底色（hue 85 附近的暖中性色）配赭石暖陶主色（hue 48 附近），不使用蓝色或蓝紫色作为主色调；钉钉等平台标识色仅在平台图标处保留。
...
- 中性色：暖纸 `--canvas`、白 `--surface`、暖灰 `--surface-subtle/--surface-hover`、暖边框 `--border/--border-strong`、暖墨色三级文字 `--text-primary/--text-secondary/--text-tertiary`。
- 主色：`--accent` 赭石暖陶，用于主按钮、链接、选中态和焦点环；`--accent-soft` 为其浅底。
- 状态色：成功绿、警告琥珀、危险红保持语义不变，soft 变体仅用于浅底徽章。
- 阴影三级：`--shadow-sm` 用于静态面板卡片，`--shadow-md` 用于浮层菜单和主按钮悬停，`--shadow-lg` 用于模态；阴影一律使用暖调（hue 60-70），不使用蓝色调阴影或厚重视觉阴影。
...
- 主操作使用赭石暖陶主色（`--accent`），普通操作使用中性按钮，危险操作只用于不可逆动作。
