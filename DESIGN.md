...
五项经营指标在主值下展示紧邻上一期、包含天数相同的环比。金额和数量显示相对百分比，退款率和毛利率显示百分点；方向必须同时使用箭头、中文和数值表达，不能只依赖红绿。上期为零、结果缺失、覆盖不完整或计算失败时展示具体不可比原因，保留本期值，不把上期异常升级为整个总览失败。趋势和平台贡献仍只表达本期销售事实。
