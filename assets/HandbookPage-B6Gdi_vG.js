import{j as n,r as d}from"./react-vendor-CgER1fZB.js";import{B as ln,M as cn,a as dn,r as pn}from"./markdown-vendor-BtF6cZKv.js";import{P as un}from"./PageHeader-BF8LF3r1.js";import{k as W}from"./index-CTvTSysb.js";import{T as x,ak as C,X as G,ab as mn,a8 as _n,ai as In,ad as z,D as Q,aI as $,b as J,R as gn,aM as B,av as An,c as hn,f as fn,Q as K}from"./icons-vendor-CkwVEAyF.js";const X=[{id:"handbook",label:"使用手册"},{id:"product",label:"产品与设计"},{id:"platform",label:"平台能力"}],k=s=>String(s??"").replace(/\s+/g," ").trim(),L=s=>/^#{1,6}\s|^(```|~~~|>|[-*+]\s|\d+[.)]\s|\|)/.test(s),V=s=>k(s.replace(/!\[([^\]]*)\]\([^)]*\)/g,"$1").replace(/\[([^\]]+)\]\([^)]*\)/g,"$1").replace(/[`*_~]/g,"")),vn=s=>{const e=String(s??"").match(/^#\s+(.+?)\s*#*\s*$/m);return e?V(e[1]):"未命名说明"},Dn=s=>{const e=String(s??"").split(/\r?\n/);for(let a=0;a<e.length;a+=1){const t=e[a].trim();if(!t||L(t))continue;const l=[t];for(;e[a+1]?.trim()&&!L(e[a+1].trim());)l.push(e[a+1].trim()),a+=1;return V(l.join(" "))}return"暂无摘要"};function w(s){const e=String(s.content??"");return{...s,content:e,title:k(s.title)||vn(e),summary:k(s.summary)||Dn(e)}}function Pn(s,{query:e="",category:a="all"}={}){const t=k(e).toLocaleLowerCase("zh-CN");return s.filter(l=>a&&a!=="all"&&l.category!==a?!1:t?[l.title,l.summary,l.content].join(`
`).toLocaleLowerCase("zh-CN").includes(t):!0)}function En(s,e,a){return s.length?s.find(t=>t.slug===e)??s.find(t=>t.slug===a)??s[0]:null}function Nn(s){const e=new ln,a=[],t=/^(#{2,3})\s+(.+?)\s*#*\s*$/gm;let l;for(;(l=t.exec(String(s??"")))!==null;){const o=V(l[2]);a.push({level:l[1].length,title:o,id:e.slug(o)})}return a}function bn(s){const e=String(s??""),a=e.split(/\r?\n/);let t=0;for(;t<a.length&&!a[t].trim();)t+=1;if(!/^#\s+/.test(a[t]?.trim()??""))return e;for(t+=1;t<a.length&&!a[t].trim();)t+=1;if(a[t]?.trim()&&!L(a[t].trim())){for(;t<a.length&&a[t].trim();)t+=1;for(;t<a.length&&!a[t].trim();)t+=1}return a.slice(t).join(`
`).trim()}const Sn=s=>/^https?:\/\//i.test(s??"");function jn({content:s}){return n.jsx("div",{className:"handbook-markdown",children:n.jsx(cn,{remarkPlugins:[pn],rehypePlugins:[dn],components:{h1:()=>null,a:({href:e,children:a,...t})=>Sn(e)?n.jsx("a",{...t,href:e,target:"_blank",rel:"noreferrer",children:a}):n.jsx("a",{...t,href:e,children:a}),table:({children:e,...a})=>n.jsx("div",{className:"handbook-table-wrap",children:n.jsx("table",{...a,children:e})})},children:s})})}const Tn=`# 公司经营功能使用说明
