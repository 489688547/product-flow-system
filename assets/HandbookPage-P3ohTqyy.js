import{j as n,r as d}from"./react-vendor-CgER1fZB.js";import{B as ln,M as cn,a as dn,r as pn}from"./markdown-vendor-BtF6cZKv.js";import{P as un}from"./PageHeader-BF8LF3r1.js";import{m as q}from"./index-Cn_Z89vq.js";import{T as O,aj as k,X as G,aa as mn,a8 as _n,ah as In,ac as Q,D as z,aJ as $,b as J,R as An,aN as K,av as gn,c as fn,f as hn,Q as B}from"./icons-vendor-CQoDgkJd.js";const X=[{id:"handbook",label:"使用手册"},{id:"product",label:"产品与设计"},{id:"platform",label:"平台能力"}],b=a=>String(a??"").replace(/\s+/g," ").trim(),L=a=>/^#{1,6}\s|^(```|~~~|>|[-*+]\s|\d+[.)]\s|\|)/.test(a),V=a=>b(a.replace(/!\[([^\]]*)\]\([^)]*\)/g,"$1").replace(/\[([^\]]+)\]\([^)]*\)/g,"$1").replace(/[`*_~]/g,"")),vn=a=>{const e=String(a??"").match(/^#\s+(.+?)\s*#*\s*$/m);return e?V(e[1]):"未命名说明"},En=a=>{const e=String(a??"").split(/\r?\n/);for(let s=0;s<e.length;s+=1){const t=e[s].trim();if(!t||L(t))continue;const l=[t];for(;e[s+1]?.trim()&&!L(e[s+1].trim());)l.push(e[s+1].trim()),s+=1;return V(l.join(" "))}return"暂无摘要"};function w(a){const e=String(a.content??"");return{...a,content:e,title:b(a.title)||vn(e),summary:b(a.summary)||En(e)}}function Dn(a,{query:e="",category:s="all"}={}){const t=b(e).toLocaleLowerCase("zh-CN");return a.filter(l=>s&&s!=="all"&&l.category!==s?!1:t?[l.title,l.summary,l.content].join(`
`).toLocaleLowerCase("zh-CN").includes(t):!0)}function Pn(a,e,s){return a.length?a.find(t=>t.slug===e)??a.find(t=>t.slug===s)??a[0]:null}function Tn(a){const e=new ln,s=[],t=/^(#{2,3})\s+(.+?)\s*#*\s*$/gm;let l;for(;(l=t.exec(String(a??"")))!==null;){const o=V(l[2]);s.push({level:l[1].length,title:o,id:e.slug(o)})}return s}function Nn(a){const e=String(a??""),s=e.split(/\r?\n/);let t=0;for(;t<s.length&&!s[t].trim();)t+=1;if(!/^#\s+/.test(s[t]?.trim()??""))return e;for(t+=1;t<s.length&&!s[t].trim();)t+=1;if(s[t]?.trim()&&!L(s[t].trim())){for(;t<s.length&&s[t].trim();)t+=1;for(;t<s.length&&!s[t].trim();)t+=1}return s.slice(t).join(`
...
店铺和数据源连接使用通用 provider registry。平台只统一连接实例、加密凭证、公司 Mac 设备、采集任务、人工验证接管、幂等、结果写入和审计；每个 provider adapter 独立定义登录字段、固定域名、任务类型、资源类型和标准事实写入器。抖音电商是首个 adapter，先采集 \`connection_identity\` 店铺 ID、名称和头像；ERP、快手、天猫、巨量等后续接入复用同一任务协议，不复制自动化框架，也不把异构事实写入万能 JSON 表。
...
平台连接使用带平台标识、名称、用途和状态的清单，不使用一张通用技术表单。点击平台后打开各自的专属字段和中文帮助；未接入平台保留可理解的说明和不可操作状态。界面只表达“已连接、需配置、需处理、未接入”等业务状态，不出现环境变量、数据库、加密算法等实现术语。普通列表和读取接口不提供已保存秘密；仅实例级店铺连接可在有管理权限、登录会话创建不超过 15 分钟、明确点击、限流、no-store 和追加审计同时成立时短时显示密码，并在 60 秒、页面失焦、Esc 或关闭时清空。

数据接入首页展示内置 provider 入口和已识别实例，不展示公司主体、链接名称、负责人、接入方式或后台地址。抖音首期弹窗只有登录邮箱和密码；成功识别后外层只显示真实店铺头像、店铺名称和短状态。
