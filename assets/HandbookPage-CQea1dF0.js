...
承担订单明细读取、日销售聚合和共享商品主数据读取。签名和密钥仅在服务端使用。订单按日期和页码执行，保留订单创建时间；商品按 \`item.list.query\` 全部分页执行，组合候选再按游标调用 \`item.single.get\` 读取 \`suitSingleList\`。当前授权的商品列表响应不单独返回库存单位编码，必须与详情和商品档案导出按主/规格商家编码合并，不能把 API 缺失解释为 ERP 删除。库存最小单位可以使用标准 69 码或 ERP 内部唯一码，不能按格式过滤。
