import{j as n,r as c}from"./react-vendor-Cdl46aim.js";import{B as ln,M as dn,r as cn,a as un}from"./markdown-vendor-CTibGXZM.js";import{P as pn}from"./PageHeader-DZv__LwJ.js";import{j as $}from"./index-BEL5NVaQ.js";import{z as T,a2 as w,X as G,O as mn,J as gn,a0 as hn,V as H,ah as q,aA as Q,aq as J,R as fn,aB as B,ai as In,af as xn,a6 as bn,l as Y}from"./icons-vendor-CVjR7iXs.js";const X=[{id:"handbook",label:"使用手册"},{id:"product",label:"产品与设计"},{id:"platform",label:"平台能力"}],E=s=>String(s??"").replace(/\s+/g," ").trim(),O=s=>/^#{1,6}\s|^(```|~~~|>|[-*+]\s|\d+[.)]\s|\|)/.test(s),V=s=>E(s.replace(/!\[([^\]]*)\]\([^)]*\)/g,"$1").replace(/\[([^\]]+)\]\([^)]*\)/g,"$1").replace(/[`*_~]/g,"")),jn=s=>{const e=String(s??"").match(/^#\s+(.+?)\s*#*\s*$/m);return e?V(e[1]):"未命名说明"},_n=s=>{const e=String(s??"").split(/\r?\n/);for(let t=0;t<e.length;t+=1){const a=e[t].trim();if(!a||O(a))continue;const l=[a];for(;e[t+1]?.trim()&&!O(e[t+1].trim());)l.push(e[t+1].trim()),t+=1;return V(l.join(" "))}return"暂无摘要"};function L(s){const e=String(s.content??"");return{...s,content:e,title:E(s.title)||jn(e),summary:E(s.summary)||_n(e)}}function vn(s,{query:e="",category:t="all"}={}){const a=E(e).toLocaleLowerCase("zh-CN");return s.filter(l=>t&&t!=="all"&&l.category!==t?!1:a?[l.title,l.summary,l.content].join(`
`).toLocaleLowerCase("zh-CN").includes(a):!0)}function An(s,e,t){return s.length?s.find(a=>a.slug===e)??s.find(a=>a.slug===t)??s[0]:null}function kn(s){const e=new ln,t=[],a=/^(#{2,3})\s+(.+?)\s*#*\s*$/gm;let l;for(;(l=a.exec(String(s??"")))!==null;){const o=V(l[2]);t.push({level:l[1].length,title:o,id:e.slug(o)})}return t}function Dn(s){const e=String(s??""),t=e.split(/\r?\n/);let a=0;for(;a<t.length&&!t[a].trim();)a+=1;if(!/^#\s+/.test(t[a]?.trim()??""))return e;for(a+=1;a<t.length&&!t[a].trim();)a+=1;if(t[a]?.trim()&&!O(t[a].trim())){for(;a<t.length&&t[a].trim();)a+=1;for(;a<t.length&&!t[a].trim();)a+=1}return t.slice(a).join(`
