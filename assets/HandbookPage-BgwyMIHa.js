import{j as n,r as d}from"./react-vendor-CgER1fZB.js";import{B as ln,M as cn,r as dn,a as pn}from"./markdown-vendor-D4QsUKML.js";import{P as un}from"./PageHeader-BF8LF3r1.js";import{k as W}from"./index-BrJntI-S.js";import{a0 as S,ae as O,X as G,a5 as mn,a2 as gn,ac as hn,a7 as H,D as Q,aF as $,ax as J,R as _n,aJ as F,aq as In,b as fn,ah as An,z as K}from"./icons-vendor-3TIvZxeF.js";const X=[{id:"handbook",label:"使用手册"},{id:"product",label:"产品与设计"},{id:"platform",label:"平台能力"}],T=s=>String(s??"").replace(/\s+/g," ").trim(),w=s=>/^#{1,6}\s|^(```|~~~|>|[-*+]\s|\d+[.)]\s|\|)/.test(s),V=s=>T(s.replace(/!\[([^\]]*)\]\([^)]*\)/g,"$1").replace(/\[([^\]]+)\]\([^)]*\)/g,"$1").replace(/[`*_~]/g,"")),vn=s=>{const e=String(s??"").match(/^#\s+(.+?)\s*#*\s*$/m);return e?V(e[1]):"未命名说明"},bn=s=>{const e=String(s??"").split(/\r?\n/);for(let a=0;a<e.length;a+=1){const t=e[a].trim();if(!t||w(t))continue;const l=[t];for(;e[a+1]?.trim()&&!w(e[a+1].trim());)l.push(e[a+1].trim()),a+=1;return V(l.join(" "))}return"暂无摘要"};function L(s){const e=String(s.content??"");return{...s,content:e,title:T(s.title)||vn(e),summary:T(s.summary)||bn(e)}}function jn(s,{query:e="",category:a="all"}={}){const t=T(e).toLocaleLowerCase("zh-CN");return s.filter(l=>a&&a!=="all"&&l.category!==a?!1:t?[l.title,l.summary,l.content].join(`
`).toLocaleLowerCase("zh-CN").includes(t):!0)}function Dn(s,e,a){return s.length?s.find(t=>t.slug===e)??s.find(t=>t.slug===a)??s[0]:null}function xn(s){const e=new ln,a=[],t=/^(#{2,3})\s+(.+?)\s*#*\s*$/gm;let l;for(;(l=t.exec(String(s??"")))!==null;){const o=V(l[2]);a.push({level:l[1].length,title:o,id:e.slug(o)})}return a}function Pn(s){const e=String(s??""),a=e.split(/\r?\n/);let t=0;for(;t<a.length&&!a[t].trim();)t+=1;if(!/^#\s+/.test(a[t]?.trim()??""))return e;for(t+=1;t<a.length&&!a[t].trim();)t+=1;if(a[t]?.trim()&&!w(a[t].trim())){for(;t<a.length&&a[t].trim();)t+=1;for(;t<a.length&&!a[t].trim();)t+=1}return a.slice(t).join(`
