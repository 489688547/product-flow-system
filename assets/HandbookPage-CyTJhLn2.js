import{j as n,r as d}from"./react-vendor-CgER1fZB.js";import{B as ln,M as cn,a as dn,r as pn}from"./markdown-vendor-BtF6cZKv.js";import{P as un}from"./PageHeader-BF8LF3r1.js";import{m as q}from"./index-D0igjH0B.js";import{T as O,ai as x,X as V,a9 as mn,a7 as _n,ag as In,ab as Q,D as z,aI as $,b as J,R as An,aM as K,au as gn,c as fn,f as hn,O as B}from"./icons-vendor-ou8jthzs.js";const X=[{id:"handbook",label:"使用手册"},{id:"product",label:"产品与设计"},{id:"platform",label:"平台能力"}],b=a=>String(a??"").replace(/\s+/g," ").trim(),L=a=>/^#{1,6}\s|^(```|~~~|>|[-*+]\s|\d+[.)]\s|\|)/.test(a),G=a=>b(a.replace(/!\[([^\]]*)\]\([^)]*\)/g,"$1").replace(/\[([^\]]+)\]\([^)]*\)/g,"$1").replace(/[`*_~]/g,"")),vn=a=>{const e=String(a??"").match(/^#\s+(.+?)\s*#*\s*$/m);return e?G(e[1]):"未命名说明"},En=a=>{const e=String(a??"").split(/\r?\n/);for(let s=0;s<e.length;s+=1){const t=e[s].trim();if(!t||L(t))continue;const l=[t];for(;e[s+1]?.trim()&&!L(e[s+1].trim());)l.push(e[s+1].trim()),s+=1;return G(l.join(" "))}return"暂无摘要"};function w(a){const e=String(a.content??"");return{...a,content:e,title:b(a.title)||vn(e),summary:b(a.summary)||En(e)}}function Dn(a,{query:e="",category:s="all"}={}){const t=b(e).toLocaleLowerCase("zh-CN");return a.filter(l=>s&&s!=="all"&&l.category!==s?!1:t?[l.title,l.summary,l.content].join(`
`).toLocaleLowerCase("zh-CN").includes(t):!0)}function Tn(a,e,s){return a.length?a.find(t=>t.slug===e)??a.find(t=>t.slug===s)??a[0]:null}function Pn(a){const e=new ln,s=[],t=/^(#{2,3})\s+(.+?)\s*#*\s*$/gm;let l;for(;(l=t.exec(String(a??"")))!==null;){const o=G(l[2]);s.push({level:l[1].length,title:o,id:e.slug(o)})}return s}function Nn(a){const e=String(a??""),s=e.split(/\r?\n/);let t=0;for(;t<s.length&&!s[t].trim();)t+=1;if(!/^#\s+/.test(s[t]?.trim()??""))return e;for(t+=1;t<s.length&&!s[t].trim();)t+=1;if(s[t]?.trim()&&!L(s[t].trim())){for(;t<s.length&&s[t].trim();)t+=1;for(;t<s.length&&!s[t].trim();)t+=1}return s.slice(t).join(`
