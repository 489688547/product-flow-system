import{j as n,r as d}from"./react-vendor-CgER1fZB.js";import{B as ln,M as cn,a as dn,r as pn}from"./markdown-vendor-BtF6cZKv.js";import{P as un}from"./PageHeader-BF8LF3r1.js";import{k as W}from"./index-CCAS1a-z.js";import{T as k,aj as C,X as G,aa as mn,a7 as _n,ah as In,ac as z,D as Q,aH as $,b as J,R as gn,aL as F,au as hn,c as An,f as fn,O as K}from"./icons-vendor-SUGyc2Gb.js";const X=[{id:"handbook",label:"使用手册"},{id:"product",label:"产品与设计"},{id:"platform",label:"平台能力"}],T=s=>String(s??"").replace(/\s+/g," ").trim(),w=s=>/^#{1,6}\s|^(```|~~~|>|[-*+]\s|\d+[.)]\s|\|)/.test(s),V=s=>T(s.replace(/!\[([^\]]*)\]\([^)]*\)/g,"$1").replace(/\[([^\]]+)\]\([^)]*\)/g,"$1").replace(/[`*_~]/g,"")),vn=s=>{const e=String(s??"").match(/^#\s+(.+?)\s*#*\s*$/m);return e?V(e[1]):"未命名说明"},Dn=s=>{const e=String(s??"").split(/\r?\n/);for(let a=0;a<e.length;a+=1){const t=e[a].trim();if(!t||w(t))continue;const l=[t];for(;e[a+1]?.trim()&&!w(e[a+1].trim());)l.push(e[a+1].trim()),a+=1;return V(l.join(" "))}return"暂无摘要"};function L(s){const e=String(s.content??"");return{...s,content:e,title:T(s.title)||vn(e),summary:T(s.summary)||Dn(e)}}function bn(s,{query:e="",category:a="all"}={}){const t=T(e).toLocaleLowerCase("zh-CN");return s.filter(l=>a&&a!=="all"&&l.category!==a?!1:t?[l.title,l.summary,l.content].join(`
`).toLocaleLowerCase("zh-CN").includes(t):!0)}function Pn(s,e,a){return s.length?s.find(t=>t.slug===e)??s.find(t=>t.slug===a)??s[0]:null}function jn(s){const e=new ln,a=[],t=/^(#{2,3})\s+(.+?)\s*#*\s*$/gm;let l;for(;(l=t.exec(String(s??"")))!==null;){const o=V(l[2]);a.push({level:l[1].length,title:o,id:e.slug(o)})}return a}function Sn(s){const e=String(s??""),a=e.split(/\r?\n/);let t=0;for(;t<a.length&&!a[t].trim();)t+=1;if(!/^#\s+/.test(a[t]?.trim()??""))return e;for(t+=1;t<a.length&&!a[t].trim();)t+=1;if(a[t]?.trim()&&!w(a[t].trim())){for(;t<a.length&&a[t].trim();)t+=1;for(;t<a.length&&!a[t].trim();)t+=1}return a.slice(t).join(`
`).trim()}const xn=s=>/^https?:\/\//i.test(s??"");function En({content:s}){return n.jsx("div",{className:"handbook-markdown",children:n.jsx(cn,{remarkPlugins:[pn],rehypePlugins:[dn],components:{h1:()=>null,a:({href:e,children:a,...t})=>xn(e)?n.jsx("a",{...t,href:e,target:"_blank",rel:"noreferrer",children:a}):n.jsx("a",{...t,href:e,children:a}),table:({children:e,...a})=>n.jsx("div",{className:"handbook-table-wrap",children:n.jsx("table",{...a,children:e})})},children:s})})}const Nn=`# 公司经营功能使用说明
