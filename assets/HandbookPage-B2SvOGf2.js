import{j as n,r as d}from"./react-vendor-CgER1fZB.js";import{B as ln,M as cn,a as dn,r as pn}from"./markdown-vendor-BtF6cZKv.js";import{P as un}from"./PageHeader-BF8LF3r1.js";import{m as q}from"./index-BqVIYyyH.js";import{T as O,aj as y,X as G,aa as mn,a8 as _n,ah as In,ac as Q,D as z,aJ as $,b as J,R as An,aN as K,av as gn,c as fn,f as hn,Q as B}from"./icons-vendor-khCmWx_w.js";const X=[{id:"handbook",label:"使用手册"},{id:"product",label:"产品与设计"},{id:"platform",label:"平台能力"}],b=s=>String(s??"").replace(/\s+/g," ").trim(),L=s=>/^#{1,6}\s|^(```|~~~|>|[-*+]\s|\d+[.)]\s|\|)/.test(s),V=s=>b(s.replace(/!\[([^\]]*)\]\([^)]*\)/g,"$1").replace(/\[([^\]]+)\]\([^)]*\)/g,"$1").replace(/[`*_~]/g,"")),vn=s=>{const e=String(s??"").match(/^#\s+(.+?)\s*#*\s*$/m);return e?V(e[1]):"未命名说明"},En=s=>{const e=String(s??"").split(/\r?\n/);for(let a=0;a<e.length;a+=1){const t=e[a].trim();if(!t||L(t))continue;const l=[t];for(;e[a+1]?.trim()&&!L(e[a+1].trim());)l.push(e[a+1].trim()),a+=1;return V(l.join(" "))}return"暂无摘要"};function w(s){const e=String(s.content??"");return{...s,content:e,title:b(s.title)||vn(e),summary:b(s.summary)||En(e)}}function Dn(s,{query:e="",category:a="all"}={}){const t=b(e).toLocaleLowerCase("zh-CN");return s.filter(l=>a&&a!=="all"&&l.category!==a?!1:t?[l.title,l.summary,l.content].join(`
`).toLocaleLowerCase("zh-CN").includes(t):!0)}function Pn(s,e,a){return s.length?s.find(t=>t.slug===e)??s.find(t=>t.slug===a)??s[0]:null}function Tn(s){const e=new ln,a=[],t=/^(#{2,3})\s+(.+?)\s*#*\s*$/gm;let l;for(;(l=t.exec(String(s??"")))!==null;){const o=V(l[2]);a.push({level:l[1].length,title:o,id:e.slug(o)})}return a}function Nn(s){const e=String(s??""),a=e.split(/\r?\n/);let t=0;for(;t<a.length&&!a[t].trim();)t+=1;if(!/^#\s+/.test(a[t]?.trim()??""))return e;for(t+=1;t<a.length&&!a[t].trim();)t+=1;if(a[t]?.trim()&&!L(a[t].trim())){for(;t<a.length&&a[t].trim();)t+=1;for(;t<a.length&&!a[t].trim();)t+=1}return a.slice(t).join(`
