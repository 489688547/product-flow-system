...
test("shared state layer uses the same-origin production data proxy during local preview", async () => {
  ...
  assert.match(store, /localStorage\.removeItem\(localKey\(DIRTY_STORAGE_KEY\)\)/);
  assert.match(store, /method: "POST"/);
  assert.match(store, /localStorage\.getItem\(localKey\(STORAGE_KEY\)\)/);
  assert.match(store, /localStorage\.setItem\(localKey\(STORAGE_KEY\)/);
  ...
  assert.match(store, /localStorage\.setItem\(localKey\(STORAGE_KEY\), JSON\.stringify\(nextState\)\)/);
  assert.match(store, /const updateTask = useCallback[\s\S]*commitState\(current/);
  assert.match(store, /localStorage\.getItem\(localKey\(STORAGE_KEY\)\) === serializedState/);
