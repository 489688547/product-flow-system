...
test("Vite build applies and enforces the shared 500 KB chunk policy", async () => {
  ...
  assert.equal(pkg.scripts.build, "npm run check:environment-capabilities && vite build && node scripts/prepare-runtime-build.mjs && node scripts/check-build-chunks.mjs");
});
