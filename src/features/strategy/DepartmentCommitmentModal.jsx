...
function defaultForm(record, currentUser) {
...
export function DepartmentCommitmentModal({ open, record, milestones = [], strategies, currentUser, orgCache, onClose, onSave }) {
  const [form, setForm] = useState(() => defaultForm(record, currentUser));
  const [items, setItems] = useState(() => milestones.length ? milestones : [emptyMilestone(currentUser?.name)]);
  const [error, setError] = useState("");
  useEffect(() => {
    const nextForm = defaultForm(record, currentUser);
    setForm(nextForm);
    setItems(milestones.length ? milestones.map(item => ({ ...item })) : [emptyMilestone(nextForm.owner)]);
    setError("");
  }, [currentUser, milestones, open, record]);
  const set = patch => setForm(current => ({ ...current, ...patch }));
  const patchMilestone = (index, patch) => setItems(current => current.map((item, itemIndex) => itemIndex === index ? { ...item, ...patch } : item));
  const save = () => {
    if (!form.title.trim() || !form.strategyId || !form.department || !form.owner) return setError("请填写承诺名称、关联战略、部门和负责人。");
  ...
  return (
    <Modal open={open} title={`${record ? "编辑" : "新建"}部门承诺`} size="wide" onClose={onClose} footer={<><Button onClick={onClose}>取消</Button><Button variant="primary" onClick={save}>保存草稿</Button></>}>
      ...
      <div className="form-grid">
        ...
        <label>关联战略<select value={form.strategyId || ""} onChange={event => set({ strategyId: event.target.value })}><option value="">选择公司战略…</option>{strategies.map(strategy => <option value={strategy.id} key={strategy.id}>{strategy.name}</option>)}</select></label>
