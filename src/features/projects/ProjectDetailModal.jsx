import { BellRing, Check, CircleHelp, Pencil, Plus, ShieldAlert, Trash2 } from "lucide-react";
...
export function ProjectDetailModal({ open, project, state, orgCache, currentUser, onClose, dispatch, archiveProjectChild, onDelete, onEdit, onSyncDecisionTodo }) {
  ...
  const startAdding = (type, record = null) => { setAdding(type); setForm(record ? { ...record } : type === "milestone" ? { critical: true, status: "pending" } : type === "risk" ? { severity: "high", status: "open" } : { status: "pending", decisionOwner: "周荣庆" }); };
  const saveSubrecord = () => {
    if (adding === "milestone" && form.title && form.owner && form.dueDate) dispatch({ type: "upsert_milestone", record: { ...form, id: form.id || `milestone-${Date.now()}`, projectId: project.id } });
    if (adding === "risk" && form.title && form.owner && form.response) dispatch({ type: "upsert_risk", record: { ...form, id: form.id || `risk-${Date.now()}`, projectId: project.id } });
    if (adding === "decision" && form.title && form.decisionOwner && form.recommendation) dispatch({ type: "upsert_decision", record: { ...form, id: form.id || `decision-${Date.now()}`, projectId: project.id, requester: form.requester || currentUser?.name || "" } });
    setAdding("");
    setForm({});
  };
  ...
  return (
    <Modal open={open} title={project.name} size="wide" onClose={onClose} footer={<><Button variant="danger" onClick={onDelete}>归档项目</Button><Button onClick={onEdit}>编辑项目</Button><Button variant="primary" onClick={onClose}>完成</Button></>}>
      ...
      <section className="project-detail-section">
        ...
        {risks.map(item => <div className="project-detail-row" key={item.id}><span className={`risk-severity ${item.severity}`}>{item.severity === "critical" ? "重大" : item.severity === "high" ? "高" : item.severity === "medium" ? "中" : "低"}</span><span><strong>{item.title}</strong><small>{item.owner} · 承诺 {item.promisedAt || "待补充"} · {item.response}</small></span>{item.status !== "closed" ? <button type="button" onClick={() => dispatch({ type: "upsert_risk", record: { ...item, status: "closed", closedResult: "已完成应对动作" }, reason: "风险关闭" })}>关闭</button> : <HealthBadge health="completed" />}<div className="row-actions"><button type="button" aria-label="编辑风险" onClick={() => startAdding("risk", item)}><Pencil size={14} /></button><button type="button" aria-label="归档风险" onClick={() => setDeleting({ type: "risks", item })}><Trash2 size={14} /></button></div></div>)}
      ...
      <ConfirmDialog open={Boolean(deleting)} title="归档项目记录" message={deleting?.item?.title || "该记录"} description="归档后不再参与项目健康度计算，历史审计记录会保留。" confirmLabel="确认归档" onClose={() => setDeleting(null)} onConfirm={() => { archiveProjectChild(deleting.type, deleting.item.id); setDeleting(null); }} />
    </Modal>
  );
}
