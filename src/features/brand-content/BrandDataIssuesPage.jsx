...
import { collaborationDraftFromBrandIssue } from "../../domain/collaborationAdapters.js";
...
export function BrandDataIssuesPage() {
  ...
  const columns = [
    ...
    { key: "actions", header: "操作", render: row => <TableActions><AppCollaborationButton draft={collaborationDraftFromBrandIssue(row)} /></TableActions> }
  ];
  return (
    <section className="page brand-content-page brand-issues-page">
      ...
      <section className="section-panel">{rows.length ? <DataTable columns={columns} rows={rows} minWidth={1080} empty="没有数据问题" className="brand-issues-table" /> : <div className="empty-state"><DatabaseZap size={22} aria-hidden="true" />当前筛选没有待处理问题。</div>}</section>
    </section>
  );
}
