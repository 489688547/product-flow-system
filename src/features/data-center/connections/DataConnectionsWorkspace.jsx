...
import { loadWebCollectionStatus, registerDouyinStore } from "../../../state/webCollectionApi.js";
...
import { DouyinStoreDialog } from "./DouyinStoreDialog.jsx";
...
export function DataConnectionsWorkspace({
  ...
  const [webCollection, setWebCollection] = useState({ stores: [] });
  ...
  async function saveDouyinStore(input) {
    await registerDouyinStore(input);
  ...
  return (
    <div className="data-connections-workspace">
      ...
      {category === "ecommerce" && webCollectionError ? <div className="connector-form-error" role="alert"><span>{webCollectionError}</span><button type="button" onClick={() => refreshWebCollection()}>重试</button></div> : null}
      ...
      <div id={`data-access-panel-${category}`} role="tabpanel" aria-labelledby={`data-access-tab-${category}`}>
        ...
        {!connectionsLoading && category === "ecommerce" ? (
          <ConnectorCatalog
            ...
            douyinStores={webCollection.stores}
            canManagePlatform={canManagePlatform}
            onAddDouyin={() => setDouyinDialogOpen(true)}
          />
        ) : null}
      ...
      {douyinDialogOpen ? (
        <DouyinStoreDialog
          onSave={saveDouyinStore}
          onClose={() => setDouyinDialogOpen(false)}
        />
      ) : null}
    </div>
  );
