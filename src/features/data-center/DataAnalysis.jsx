...
export function DataAnalysis({ rows, range, productNames }) {
  ...
  const maxDayNetSales = useMemo(() => Math.max(...summary.byDay.map(item => item.netSales), 1), [summary]);
  const platformOptions = useMemo(() => [{ value: "all", label: "全部平台" }, ...platforms.map(value => ({ value, label: value }))], [platforms]);
  const productOptions = useMemo(() => [{ value: "all", label: "全部商品" }, ...products.map(value => ({ value, label: productNames.get(value) || value }))], [products, productNames]);
  const columns = [
    ...
    { key: "qty", header: "销售数量", className: "num", render: row => number(row.qty) },
    { key: "netSales", header: "净销售额", className: "num", render: row => money(row.netSales) },
    { key: "refund", header: "退款", className: "num", render: row => money(row.refund) },
    { key: "grossProfit", header: "毛利", className: "num", render: row => money(row.grossProfit) }
  ];
  ...
  return (
    <div className="data-workspace">
      <section className="data-analysis-toolbar" aria-label="分析筛选">
        <div><span>平台筛选</span><HeaderFilter label="平台" value={platform} onChange={setPlatform} options={platformOptions} /></div>
        <div><span>商品筛选</span><HeaderFilter label="商品" value={code} onChange={setCode} options={productOptions} /></div>
