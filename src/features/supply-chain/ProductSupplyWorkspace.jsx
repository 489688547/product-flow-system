    ...
    <section className="supply-flat-workspace">
      {canEdit ? (
        <div className="supply-workspace-toolbar">
          <Button variant="primary" onClick={() => setOpen(true)}><Plus size={16} />关联供应商</Button>
        </div>
      ) : null}
      <DataTable columns={columns} rows={state.productSupplierLinks} minWidth={960} empty={<div className="empty-state compact-empty">还没有产品供应关系。可按现有成本表逐条导入或维护。</div>} />
      <Modal
        title="关联产品供应商"
        open={open}
        ...
        footer={<>
          ...
          <Button variant="primary" disabled={!form.productId || !form.supplierId} onClick={save}>保存</Button>
        </>}
      >
        <div className="form-grid supply-form-grid">
          <label>产品<select value={form.productId} onChange={event => setForm(current => ({ ...current, productId: event.target.value }))}><option value="">请选择</option>{products.map(product => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label>
          ...
          <label>类别<select value={form.category} onChange={event => setForm(current => ({ ...current, category: event.target.value }))}><option>原料</option><option>包材</option><option>里料</option><option>耗材</option><option>加工</option><option>成品</option></select></label>
          ...
          <label>物料名称<input value={form.materialName} onChange={event => setForm(current => ({ ...current, materialName: event.target.value }))} /></label>
