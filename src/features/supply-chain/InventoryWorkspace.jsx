...
export function InventoryWorkspace({
  ...
  const snapshotColumns = [
    ...
    { key: "erp", header: <span className="num">ERP库存</span>, render: row => <span className="num">{Number(row.erpQuantity || 0).toLocaleString("zh-CN")}</span> },
    { key: "counted", header: <span className="num">实盘库存</span>, render: row => <span className="num">{row.countedQuantity === null || row.countedQuantity === undefined ? "—" : Number(row.countedQuantity).toLocaleString("zh-CN")}</span> },
    { key: "variance", header: <span className="num">盘点差异</span>, render: row => <span className="num">{row.quantityVariance === null || row.quantityVariance === undefined ? "—" : <span className={row.quantityVariance ? "text-warning" : ""}>{row.quantityVariance > 0 ? "+" : ""}{row.quantityVariance}</span>}</span> },
    { key: "amount", header: <span className="num">实盘金额</span>, render: row => <span className="num">{row.inventoryAmount === null ? "—" : `¥${Number(row.inventoryAmount).toFixed(2)}`}</span> }
  ];
  ...
  const projectionColumns = [
    ...
    { key: "erp", header: <span className="num">ERP账面</span>, render: row => <span className="num">{displayNumber(row.erpQuantity)}</span> },
    ...
    { key: "funds", header: <span className="num">库存资金</span>, render: row => <span className="num">{displayMoney(row.inventoryCashTied)}</span> },
  ...
  const riskColumns = [
    ...
    { key: "days", header: <span className="num">预计可售</span>, render: row => <span className="num"><strong className={Number(row.sellableDays) <= 5 ? "text-warning" : ""}>{row.sellableDays} 天</strong></span> },
  ...
  const materialColumns = [
    ...
    { key: "warehouse", header: "仓库", render: row => row.warehouse || "—" },
    { key: "quantity", header: <span className="num">数量</span>, render: row => <span className="num">{Number(row.quantity || 0).toLocaleString("zh-CN")}</span> },
    { key: "unitCost", header: <span className="num">单价</span>, render: row => <span className="num">{row.unitCost === null ? "—" : `¥${Number(row.unitCost).toLocaleString("zh-CN", { maximumFractionDigits: 4 })}`}</span> },
    { key: "amount", header: <span className="num">库存金额</span>, render: row => <span className="num">{`¥${Number(row.inventoryAmount || 0).toLocaleString("zh-CN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}</span> },
  ...
  return <div className="supply-work-grid">
    <div className="goods-flow-headlines" aria-label="库存概览">
      ...
      <div className="goods-flow-headline">
        <span>待盘点 SKU</span>
        <strong>{pendingStocktakeCount}<small>个</small></strong>
        <p>首次月度盘点前标记为“未盘点”，不直接沿用 ERP 账面</p>
      </div>
      <div className="goods-flow-headline">
        <span>异常库存</span>
        <strong>{activeRiskCount}<small>条处理中</small></strong>
        <p>{activeRiskCount ? "优先跟进供应商到货与补货处理" : "当前没有异常中的库存记录"}</p>
      </div>
    </div>
    <nav className="supply-procurement-switcher" aria-label="库存工作区分组导航">
    ...
    <section className="section-panel">
      <div className="section-head">
        <div><h2>成品库存与盘点轨迹</h2><p>导入只写入 ERP 账面快照；实盘必须经过上方月度盘点流程，不能在导入时直接确认金额。每次快照和实盘结果独立保留。</p></div>
        {canEdit ? <label className={`upload-field ${parsing ? "is-busy" : ""}`}><Upload size={16} />{parsing ? "正在解析…" : "选择 XLSX / CSV"}<input type="file" accept=".xlsx,.csv" disabled={parsing} onChange={event => { handleFile(event.target.files?.[0]); event.target.value = ""; }} /></label> : null}
      </div>
