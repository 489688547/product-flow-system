...
export function TaskTemplateSettings({ templates, orgCache, onSave, canEdit = false }) {
  ...
  const columns = [
    ...
    { key: "documents", header: "交付物模板", render: template => <Button disabled={!canEdit} className="compact" onClick={() => setDocumentTaskId(template.id)}><FileText size={15} />模板 {template.deliverableTemplates?.length || 0}</Button> },
    ...
    <section className="section-panel settings-task-templates">
      <div className="section-head settings-template-head">
        ...
        <div className="settings-save-actions">
          ...
          {canEdit ? <Button variant="primary" disabled={!hasChanges} onClick={handleSave}><Save size={16} />保存任务配置</Button> : null}
        </div>
      </div>
