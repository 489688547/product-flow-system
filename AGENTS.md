...
- The supported local runtime is `npm start`: Vite is the single browser entry and proxies `/api` to a local Pages-compatible Functions process backed only by local SQLite. Initialize it with `npm run seed:sandbox`; it must not read production tokens, remote databases or production provider credentials. Shared acceptance uses the fixed test URLs, and production verification uses the ECS production URL. Never restore a hard-coded local identity, remote D1 access, or a local transport that impersonates a production account.
