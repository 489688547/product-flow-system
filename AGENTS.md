## Imported Claude Cowork project instructions

## Source of truth

- `PRODUCT.md` and `docs/product/` define durable product intent, workflows, roles, and data definitions.
- `DESIGN.md` defines durable interface rules. Feature-specific interaction decisions live beside the feature PRD.
- `docs/platform/` defines shared components, middleware, API contracts, integrations, and errors.
- `docs/decisions/` records architecture decisions and their consequences.
- Code and executable tests must remain consistent with these documents. When behavior changes, update the durable source in the same pull request.

## Architecture boundaries

- `src/domain`: pure business rules; no React, browser globals, or network requests.
- `src/ui`: business-neutral reusable UI components.
- `src/features`: page and feature composition; reuse domain, state, and UI boundaries.
- `src/state`: application orchestration and API clients; no visual component definitions.
- `functions/api`: authenticated server routes and provider adapters; reuse shared middleware.
- `docs`: product, design, platform, feature, and decision sources of truth.

Dependency direction is `features -> ui/domain/state`, `state -> API clients`, and `functions/api routes -> shared middleware/provider adapters`. Features must not call DingTalk, Kuaimai, ERP, or other external systems directly.

Keep changes small and focused. When modifying a high-touch file that is already difficult to understand, extract the responsibility being changed instead of adding another unrelated branch. Do not perform broad refactors without a reviewed plan.

## Feature workflow

Medium or larger work requires `docs/features/<feature>/prd.md`, `design.md`, `plan.md`, and `tasks.md`.

1. Research the current code, tests, product rules, and runtime boundary.
2. Define the problem, non-goals, business rules, data definitions, and acceptance criteria in the PRD.
3. Define interaction states and component reuse in the design document.
4. Define files, interfaces, migration, rollback, and verification in the plan.
5. Write or update a failing test before implementation.
6. Implement one coherent task at a time and keep its task checkbox current.
7. Update durable product, design, platform, API, or decision documentation when rules change.

Small, low-risk fixes may omit feature documents only when the pull request explains why the change is self-contained and does not alter product behavior, architecture, permissions, data shape, or external contracts.

## Shared capability rules

- A UI component becomes shared when it is a base control or has at least two real consumers.
- Shared components use business-neutral APIs and cover applicable loading, empty, error, disabled, focus, and permission states.
- Middleware has one cross-cutting responsibility and documents input, output, ordering, side effects, timeout, retry, idempotency, and logs.
- Existing APIs remain internal until their contract is documented. New multi-system APIs use `/api/platform/v1/...`.
- API changes document authentication, authorization, request, response, errors, compatibility, deprecation, observability, and contract tests.
- Database or persisted-state changes require migration, backwards compatibility, capacity impact, and rollback notes.
- Environment or provider configuration changes must update `docs/platform/environment-capabilities.json`, regenerate platform modules, and pass `npm run check:environment-capabilities` in the same pull request.
- A reusable component, middleware, skill, API, integration or environment rule discovered during feature work must be written back to `AGENTS.md`, `docs/platform/`, the integration registry or an ADR before merge; feature-only notes are not a durable rule.
- Every business AI capability must first register a server-owned `appId` and `featureId`, then call the shared `invokeAiFeature` boundary. 业务 App 和浏览器不得直接调用 Provider，也不得持有模型 URL、Secret、Authorization header 或 Responses adapter；unknown features fail closed with `AI_FEATURE_NOT_REGISTERED`. The shared platform-connection tester is the only non-AI-route exception and may use the low-level adapter solely with synthetic content to validate a candidate credential.
- AI usage audit records stable App, feature, execution mode, Provider-call flag, token counts and safe result codes. Prompts, answers, business context, Skill argument values and Provider raw responses are never audited. The Data Center AI usage page may show company aggregates by App, feature, model and Skill, but never employee rankings or personal drill-downs.
- Business D1 access in authenticated routes must use the middleware-selected `context.data.businessDb` through `businessDatabase(context)` or a shared storage adapter that receives it explicitly. Direct `PRODUCT_FLOW_DB` access is limited to documented control-plane paths such as authentication, credentials, environment control, collection job control, production repair and AI control audit.
- Every new persisted table must declare a display-data catalog policy (`copy`, `mask`, `transform_sales`, `recalculate` or `skip`) before merge. Unknown tables fail closed as `skip`; personally identifying fields require deterministic masking and credentials, sessions, tokens and raw provider responses are never copied.
- Every acquisition job must persist the server-resolved target environment and version. Browsers, runners and provider payloads may not choose a binding or database ID; a stale display version fails before projection writes.
- Every external write capability must declare its display behavior. Display mode defaults to fail closed and may proceed only through a registered simulator that preserves validation, authorization, idempotency and safe audit without loading Provider credentials.
- AI Provider configuration, credentials, leases, token usage and Skill audit always use the control database. AI context builders and business Skills use the selected `businessDb`, and both usage tables record `data_environment` independently from `execution_mode`.
- Confirmed product-development work must be recorded in the shared 研发待办 control plane. Before changing an overlapping module or repository path, an agent or developer must first 查询 the confirmed backlog item and 认领 it with the real branch name; active scope conflicts must be resolved rather than bypassed. AI discussions remain drafts until an authorized executive confirms the structured item.

## Security and external systems

- Before changing environment variables, bindings, D1 tables, production-data access, Cloudflare deployment behavior, or provider configuration, use `.agents/skills/environment-parity/SKILL.md`. A request to skip review does not waive automated gates, migration, rollback, or deployed production verification.
- Before coding, debugging, reviewing, or documenting work that may touch an external platform, use `.agents/skills/integration-router/SKILL.md` and route the task through `docs/platform/integration-registry.json`.
- Treat prompt keywords as advisory. A changed path matched by the registry is mandatory and must be declared in the pull request with `Integration-Impact` and `Integration-Impact-Reason`.
- Every pull request must declare `Rule-Writeback` and `Rule-Writeback-Reason`. Shared API, integration, environment, migration, error-contract, production-gateway, or provider-boundary changes must update and name an applicable durable rule file; CI rejects an ungrounded `none`. Declarable durable rule files are `AGENTS.md`, `DESIGN.md`, `PRODUCT.md`, `.agents/skills/`, `docs/product/`, `docs/platform/`, and `docs/decisions/`. Run `npm run check:pr` before pushing to validate both declarations locally with the same rules CI applies, passing `-- --base origin/dev` for feature branches so the preflight compares against the branch the pull request actually targets rather than `origin/main`; A dev → main release leaves a merge commit on `main` that `dev` does not have, which makes the next release PR fail `check:branch-base`. The `sync-main-to-dev` workflow opens the sync pull request automatically on every push to `main`; do not push to `dev` directly to fix this, and resolve conflicts in that pull request rather than force-pushing. CI reads the pull request event payload, so editing the body or retargeting the base branch requires a new run triggered by a push — `gh run rerun` replays the stale payload and will keep failing on the old value.
- `dev` is the fixed test-candidate branch and must always contain the latest `main`. New `codex/*` feature branches start from the latest `dev`, update from `dev` before merge, and submit PRs only to `dev`; the only PR allowed to target `main` is `dev → main`. CI must run the branch-flow and branch-base gates and reject stale or cross-lane changes. Fixed acceptance sites are `https://deshan-tiyes-system-dev.pages.dev` for `dev` and `https://deshan-tiyes-system.pages.dev` for `main`; arbitrary Pages Preview URLs do not count as user acceptance.
- Never commit credentials, access tokens, cookies, personal mobile numbers, or raw provider responses containing sensitive data.
- Company-wide provider credentials must be managed through the 平台连接保险箱. 普通浏览器状态、普通 API 响应、日志和审计记录不得暴露任何明文凭据. An instance-scoped secret may be returned only by an explicit POST reveal or one-time browser-agent credential route with server-side authorization, fresh session or device-bound grant, no-store headers, rate limit, short expiry and append-only no-secret audit. 公司级 Provider 凭据如产品明确要求查看，只能由登记到平台集成注册表的专用 POST reveal 路由返回，并同时要求最高权限身份、15 分钟内新会话、用途、确认短语、频率限制、`no-store`、无秘密审计和浏览器短时内存清除；legacy environment fallback values must never be revealed. Validate candidate credentials with a read-only provider request before switching the active version; provider adapters must resolve credentials through the shared vault resolver, with legacy environment variables used only as a fallback.
- Browser and API acquisition is a shared provider capability. New ERP, ecommerce or advertising sources must register provider ID, fixed origins, task types, resource types, schema version and a standard-fact writer; reuse shared device auth, task grants, retries, cursors and audit. Provider-specific selectors and API mappings stay in adapters, and unregistered providers or resources fail closed.
- ERP inventory-unit codes are identity values, not format validators: standard 69 codes and internal unique codes are both valid when non-empty and unambiguous. Bundle type and component ratios must come from provider product type/detail data or a confirmed import, never from product names, code prefixes, or code length.
- Company documents remain behind the existing authenticated application boundary even when all employees may view them.
- Validate authorization on the server; hidden UI is not an authorization boundary.
- Keep local code execution, Cloudflare deployment, DingTalk embedded WebView, and external-provider verification as separate verification lanes; local online mode may use the same production account and resources without implying that undeployed code was verified in production.
- The supported full local runtime is `npm start`: Vite is the single browser entry and proxies `/api` to a Pages Functions bundle running with `wrangler dev --remote`, so API code executes on Cloudflare and accesses the governed D1 binding natively. A per-start server-only transport secret and hashed personal token resolve an active executive organization identity without exposing either value to the browser. This real session may use production data and execute DingTalk, Kuaimai, or other provider actions only through the same route authorization and adapters used online. Never fall back to a hard-coded local identity or `wrangler pages dev` with a remote D1 binding.
- 测试性、试验性数据写入必须使用本地沙箱（`npm run seed:sandbox` 后 `npm run start:sandbox`，本地 SQLite，不触生产库；`seed:sandbox -- --with-state` 可只读复制共享业务状态）；本地线上模式与 Preview 只用于真实业务操作与真实能力验证。详见 `docs/decisions/2026-07-22-sandbox-as-test-environment.md`。
- 展示数据库不是测试环境。它是从正式业务数据受控生成的独立 `DEMO_FLOW_DB`，仅用于对外展示和真实能力验证；刷新期间展示业务整体维护，失败时保持不可用且不得回退正式业务库。详见 `docs/decisions/2026-07-23-display-data-environment.md`。
- The production-data repair gateway remains a separate operational path: cross-environment gateway writes require a hashed personal token, active executive identity, 15-minute unlock, version check, snapshot and audit. Database access alone never bypasses provider-route authorization.
- Whole-state company persistence such as `/api/state` is server-authoritative: clients must successfully read the current server baseline before writing, every write must carry that exact baseline, and the server must reject missing or stale baselines before saving. The baseline comparison, revision advance and state-part replacement must run in one atomic database transaction. Clients must fingerprint the normalized business state and skip unchanged writes, including changes limited to volatile cache timestamps. Browser dirty caches and code defaults may be preserved locally for recovery but must never auto-initialize or overwrite shared data. Every accepted whole-state write requires a write-before snapshot and audit record.
- DingTalk organization sync may refresh names, departments, inferred roles, and active status, but a partial snapshot must never downgrade or deactivate an account already explicitly elevated to `executive`. Executive revocation requires a direct identity check or explicit administrative action.
- Cloudflare Git deployments currently bundle Pages Functions with Wrangler 3.x. Changed Functions must remain compatible with that builder, keep `tests/cloudflare-pages-compat.test.mjs` green, and be verified in both the Git deployment and the current Wrangler CLI before release.
- Production delivery is GitOps-only: development changes first merge into `dev`, pass the fixed test-site smoke and user acceptance, then release through the sole `dev → main` PR. The configured Cloudflare Git integrations deploy `dev` to `deshan-tiyes-system-dev` and `main` to `deshan-tiyes-system`; a release is complete only after the target fixed site reports the expected commit and passes OAuth/readiness verification. A direct Wrangler/Pages upload may be used only for explicitly authorized incident recovery or temporary verification and never counts as delivered.
- Do not publish, deploy, send DingTalk actions, or change remote repository settings without the required authorization.

## Definition of done

Run all of the following from the repository root:

```bash
npm run lint
npm run check:governance
npm run check:integrations
npm run check:environment-capabilities
npm test
npm run build
```

UI work also requires keyboard, focus, empty/error/disabled states, real laptop width, responsive layout, WCAG AA contrast, and DingTalk WebView review. API work requires authentication, permission, failure, timeout, and compatibility coverage.

Before committing, inspect `git status --short` and stage only files belonging to the current task. Existing user changes and unrelated worktrees must never be overwritten or included.
