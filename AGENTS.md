...
- Business D1 access in authenticated routes must use the middleware-selected `context.data.businessDb` through `businessDatabase(context)` or a shared storage adapter that receives it explicitly. Direct `PRODUCT_FLOW_DB` access is limited to documented control-plane paths such as authentication, credentials, environment control, collection job control, production repair and AI control audit.
- Every new persisted table must declare a display-data catalog policy (`copy`, `mask`, `transform_sales`, `recalculate` or `skip`) before merge. Unknown tables fail closed as `skip`; personally identifying fields require deterministic masking and credentials, sessions, tokens and raw provider responses are never copied.
...
- Every pull request must declare `Rule-Writeback` and `Rule-Writeback-Reason`. Shared API, integration, environment, migration, error-contract, production-gateway, or provider-boundary changes must update and name an applicable durable rule file; CI rejects an ungrounded `none`. Declarable durable rule files are `AGENTS.md`, `DESIGN.md`, `PRODUCT.md`, `.agents/skills/`, `docs/product/`, `docs/platform/`, and `docs/decisions/`. Run `npm run check:pr` before pushing to validate both declarations locally with the same rules CI applies, passing `-- --base origin/dev` for feature branches so the preflight compares against the branch the pull request actually targets rather than `origin/main`. CI reads the pull request event payload, but the `quality` workflow subscribes to the `edited` event type, so correcting a declaration in the pull request body re-runs the gate on its own; a new push is not required. `gh run rerun` still replays the stale payload and must not be used to re-validate an edited body.
- `dev` is the fixed test-candidate branch. The invariant is that `main` must never hold a change `dev` lacks — not that `dev` must contain every `main` commit. A `dev → main` release merge commit carries the tree of the `dev` commit it merged, so it introduces nothing new and the release-lane branch-base gate tolerates it by design; a change that reached `main` any other way fails that gate and must be merged back into `dev` before the next release. New `codex/*` feature branches start from the latest `dev`, update from `dev` before merge, and submit PRs only to `dev`; the only PR allowed to target `main` is `dev → main`. CI must run the branch-flow and branch-base gates and reject stale or cross-lane changes. Fixed acceptance sites are `https://test.deshan-tiyes.cn` with API `https://api-test.deshan-tiyes.cn` for `dev`, and `https://deshan-tiyes.cn` for `main`; arbitrary Pages Preview URLs do not count as user acceptance.
...
- Company-wide provider credentials must be managed through the 平台连接保险箱. 普通浏览器状态、普通 API 响应、日志和审计记录不得暴露任何明文凭据. An instance-scoped secret may be returned only by an explicit POST reveal or one-time browser-agent credential route with server-side authorization, fresh session or device-bound grant, no-store headers, rate limit, short expiry and append-only no-secret audit. 公司级 Provider 凭据如产品明确要求查看，只能由登记到平台集成注册表的专用 POST reveal 路由返回，并同时要求最高权限身份、15 分钟内新会话、用途、确认短语、频率限制、`no-store`、无秘密审计和浏览器短时内存清除；legacy environment fallback values must never be revealed. Validate candidate credentials with a read-only provider request before switching the active version; provider adapters must resolve credentials through the shared vault resolver, with legacy environment variables used only as a fallback.
...
- ERP inventory-unit codes are identity values, not format validators: standard 69 codes and internal unique codes are both valid when non-empty and unambiguous. Bundle type and component ratios must come from provider product type/detail data or a confirmed import, never from product names, code prefixes, or code length.
...
- Keep the local SQLite sandbox, Cloudflare static test frontend, ECS test API, ECS production, DingTalk embedded WebView, and external-provider verification as separate verification lanes. Success in one lane never implies success in another.
- The supported local entry is `npm start`. Without exactly one personal access file in `~/.config/EC-management-system/` (or the legacy `~/.config/product-flow-system/developer.env`), it runs Vite, local Functions and local SQLite with zero secrets. With one owned regular file, the Node launcher automatically restricts it to mode 0600, runs only the local Vite frontend and proxies `/api` to the deployed ECS production API; the Node proxy injects the personal `core_developer` token and the browser never receives it. `npm run start:sandbox` always forces local SQLite. Core mode is for real production operations and deployed-backend verification; undeployed Functions changes must use the sandbox or the fixed ECS test deployment. Never restore a hard-coded identity, direct remote database access, a shared developer token or a browser-visible production token.
- A `core_developer` token is personal, bound to one active stable DingTalk `userId/unionId`, stored only as a hash, independently auditable and revocable, and may grant an effective executive data role regardless of the member's display role. It does not grant credential reveal or bypass route-level provider authorization. Issuance must write a mode-0600 file outside every Git worktree and must never print or commit the raw token.
- 测试性、试验性数据写入必须使用本地沙箱（`npm run seed:sandbox` 后 `npm run start:sandbox`，本地 SQLite，不触生产库；`seed:sandbox -- --with-state` 可只读复制共享业务状态）；本地线上模式与 Preview 只用于真实业务操作与真实能力验证。详见 `docs/decisions/2026-07-22-sandbox-as-test-environment.md`。
- 展示数据库不是测试环境。它是从正式业务数据受控生成的独立 `DEMO_FLOW_DB`，仅用于对外展示和真实能力验证；刷新期间展示业务整体维护，失败时保持不可用且不得回退正式业务库。详见 `docs/decisions/2026-07-23-display-data-environment.md`。
...
- Whole-state company persistence such as `/api/state` is server-authoritative: clients must successfully read the current server baseline before writing, every write must carry that exact baseline, and the server must reject missing or stale baselines before saving. The baseline comparison, revision advance and state-part replacement must run in one atomic database transaction. Clients must fingerprint the normalized business state and skip unchanged writes, including changes limited to volatile cache timestamps. Browser dirty caches and code defaults may be preserved locally for recovery but must never auto-initialize or overwrite shared data. Every accepted whole-state write requires a write-before snapshot and audit record.
...
- Changed Functions must remain compatible with the ECS workerd/Wrangler runtime and keep `tests/workerd-runtime-compat.test.mjs` green. Cloudflare deployment is static-test-only and must reject Functions, D1 bindings, `.dev.vars`, Wrangler configuration and server secrets from its upload artifact.
- Production delivery is GitOps-only: development changes first merge into `dev`, deploy the static frontend to the fixed test site and the isolated API/SQLite runtime to ECS, then pass smoke and user acceptance. Release proceeds only through the sole `dev → main` PR and deploys the same commit to ECS production. A release is complete only after the target fixed domain reports the expected commit and passes OAuth/readiness verification; direct backend deployment outside this flow is incident recovery only. Production deployment itself is asynchronous and owned outside this repository: the image must reach the fixed ACR `main` tag, and the ECS rollout timer polls it every two minutes before backing up and replacing the container. No repository workflow performs that deployment, so a green pipeline never means production advanced. The fixed-site smoke must therefore wait a window matched to that chain rather than to a human, and `production-drift` independently compares the production `pfs-release-commit` against `main` on a schedule. A `stale`, `unreachable`, or `unknown` drift result means the image build, the ACR push, or the ECS rollout is broken and must be resolved before the next release; it must never be silenced by widening the grace period.
