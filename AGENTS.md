...
- Every pull request must declare `Rule-Writeback` and `Rule-Writeback-Reason`. Shared API, integration, environment, migration, error-contract, production-gateway, or provider-boundary changes must update and name an applicable durable rule file; CI rejects an ungrounded `none`. Declarable durable rule files are `AGENTS.md`, `DESIGN.md`, `PRODUCT.md`, `.agents/skills/`, `docs/product/`, `docs/platform/`, and `docs/decisions/`. Run `npm run check:pr` before pushing to validate both declarations locally with the same rules CI applies; CI reads the pull request body, so a body edit requires a new run rather than `gh run rerun`.
