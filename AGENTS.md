...
- Production delivery is GitOps-only: development changes must pass review and required checks, merge into GitHub `main`, and let the configured Cloudflare Git integration create the production deployment. A direct Wrangler/Pages upload may be used only for explicitly authorized incident recovery or temporary verification; it does not count as delivered until the same source is merged into `main`, the Git-triggered deployment succeeds, and production verification passes.
