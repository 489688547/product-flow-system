...
在 `DESIGN.md` 的侧栏规则中写明六个业务 App 的固定顺序；将 `docs/features/data-center-app/prd.md` 与 `docs/features/data-center-app/design.md` 中旧的“数据中心位于产品全周期之后”改为“数据中心位于业务 App 列表最后”，并保留平台分组在其后。

- [ ] **Step 5: 运行导航测试确认通过**

Run: `node --test react-tests/sidebar-navigation.test.mjs`

Expected: PASS，5 个导航测试全部通过。

- [ ] **Step 6: 提交导航任务**

```bash
git add src/App.jsx react-tests/sidebar-navigation.test.mjs DESIGN.md docs/features/data-center-app/prd.md docs/features/data-center-app/design.md
