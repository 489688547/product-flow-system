...
快麦《销售主题分析-按订单商品明细》进入共享销售事实前按来源字段归一化：`qty = 销售数量 - 退货数量`、`net_sales = 销售金额 - 退款金额`、`cost = 销售成本 - 退货成本`、`gross_profit = net_sales - cost`。标准 69 码优先取规格商家编码；不符合 69 码规则的行进入覆盖统计，不猜测映射。已有明确 `净销量`、`净销售额`、`净毛利` 的新版明细直接使用来源净值，不重复扣减。
