...
由数据中心维护、产品全周期、供应链和货流共同读取的 ERP 商品目录。商品以来源和主商家编码形成稳定身份，保留来源系统商品 ID；单品规格以商品身份和规格商家编码形成稳定身份，保留来源系统 SKU ID。规格编码可以是标准 69 码，也可以是 ERP 内部唯一码（例如测试商品使用的 `1111`）；非标准格式不是异常，只有缺失或跨商品冲突才进入质量队列。商品使用 `productKind` 区分单品与组合品，`components[]` 保存组合品的组件规格编码、正整数组成数量、可用成本和来源身份。组合品不要求拥有独立库存 SKU；有有效 `components[]` 时，`skus=[]` 不构成档案异常。组合品的展示分类固定为“组合品”，ERP 原始分类作为来源字段保留；组合成本按当前目录中各组成 SKU 的单位成本乘组成数量汇总，不使用组合品自身 SKU 成本，任一组件成本缺失时总成本为空并标记不完整。

快麦商品当前使用三类官方导出快照：普通商品提供主商品、规格、规格商家编码、成本、分类与品牌；套件和组合装提供父商品及官方子商品商家编码、组合比例。公司 Mac Chrome 插件依次获取三类文件并按稳定身份合并，人工官方文件导入作为回退；任一单类快照失败都不能把本次刷新标记为完成，也不能据此证明商品已删除。组合关系只接受官方字段，不根据名称、编码格式或编码长度猜测比例。
