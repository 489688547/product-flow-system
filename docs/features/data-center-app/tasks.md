...
- [x] 数据总览五项 KPI 已切换为读取共享口径结果；正常结果只展示主值与环比，缺失、失败和覆盖不足仍显式说明。经营趋势与平台分布跟随已确认范围并统一按 GMV 展示，日柱提供完整经营明细。
- [x] 2026-07-22 总览精简已完成真实远程 D1 验收：当月默认范围、五项上期同版环比、页头最新事实日期、GMV 趋势/平台分布、键盘日柱明细及 900/640/390px 无横向溢出均通过；完整 lint、治理、集成、环境、测试和构建门禁通过。
...
- [x] 抖店登录页适配代码已修正：固定 Chrome Profile 中从手机页切换邮箱登录并预填，登录提交、协议、邮箱验证码与风控由本人完成；采集器持续等待且不把手机页“发送验证码”误报为挑战。
- [ ] 抖音真实登录/人工验证验收完成后勾选。
