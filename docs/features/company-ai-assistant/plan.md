...
交付一个面向全公司的只读 AI 总助：通过服务端权限与数据目录跨 App 分析，首期接入灵算 Responses Provider，并在数据中心管理安全配置和外发策略；增量交付可滚动对话、Enter 发送和按需调用其他 App 的只读 Skill。
...
9. 修复输入和滚动行为，覆盖模型就绪与未就绪布局。
10. 建立只读 Skill 注册表并接入公司经营、产品、供应链和数据中心。
11. 接入店铺运营、品牌内容与绩效的现有权限投影。
12. 扩展 Responses Function Calling、受控循环、调用审计和 SSE 活动事件。
13. 在总助 UI 展示 Skill 状态，完成全量、浏览器与生产验收。
