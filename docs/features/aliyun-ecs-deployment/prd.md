...
## 自动发布与备份保留补充（DEV-000018）

- ECS 每两个日历分钟检查一次已登记的 ACR `main` 镜像；镜像 ID 未变化时不得
  生成备份、停止测试容器或重启生产。
- 新镜像必须先静态提取并校验仓库内的 Compose 合同。校验过程只能 `create` 和
  `cp`，不得启动或执行候选镜像代码；合同变化必须失败关闭并由运维先更新主机。
- 替换生产前必须完成正式库与展示库的一致性快照、清单校验和私有 OSS 上传。
  只有上传成功后才允许清理本地旧快照，本地最终只保留最新一份，历史全部保留在 OSS。
- 发布时仅临时停止按需测试容器并重建生产容器。生产必须在 60 秒内健康，否则
  自动恢复上一个镜像；无论成功失败，都要恢复原本运行的测试容器。
- 自动发布不得新增公共 webhook、长期 GitHub 高权限 Token、AccessKey、数据库表
  或 Cloudflare 后端。首次安装、合同变更和自动回滚失败仍由运维人工处理。

验收必须分别证明：相同镜像零副作用、新镜像备份先于替换、失败健康检查完成回滚、
本地只有一个快照目录、OSS 历史仍存在，以及公网 commit、匿名会话、阿里云和钉钉
readiness 均符合目标生产版本。
