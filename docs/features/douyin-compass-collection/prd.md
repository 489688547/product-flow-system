...
1. 插件在固定抖店“店铺管理”页读取店铺 ID 和店铺名，经本机 runner 校验后登记店铺目录；不上传页面正文。
2. 05:00 后服务端为已登记店铺生成四个昨天任务，并持久化目标环境和版本。
3. 公司 Mac runner 领取任务，插件在固定抖店/罗盘来源校验登录态、店铺身份、页面类型和业务日。
4. 有官方报表则触发下载；`store_daily` 无报表时只读取固定白名单指标。
5. 本机完成文件稳定性检查、原始归档、解析、脱敏、业务日核对和标准化预检。
6. 事实按批次写入目标业务 D1；只有完整批次提交后任务才成功并推进资源游标。
7. 登录、验证码、滑块、扫码或设备确认进入 `waiting_human`；员工处理后重试原任务。
...
3. 未登记店铺可由已登录插件自动识别并登记，店铺目录只保存稳定 ID、显示名、runner 和最近识别时间。
...
7. 数据接入页显示“Chrome 官方报表采集”的真实 readiness，巨量千川仍为未接入。
8. 首次真实验收至少完成一个已登录店铺的昨天四资源探测，并记录每个资源的实际结果与恢复动作。
9. 全部治理、集成、环境、测试和构建门禁通过；GitHub main 的 GitOps 部署和生产验证通过后才算交付。
