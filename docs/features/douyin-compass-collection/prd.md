...
1. 数据负责人可先在数据接入页登记店铺名称和店铺 ID；插件也可在固定抖店“店铺管理”页读取同一身份，经本机 runner 校验后幂等更新目录；两条路径都不上传页面正文。
...
- 不同抖音登录账号使用独立 Chrome Profile；同一账号下可切换的多个店铺可共用 Profile。扩展领取抖音任务时必须携带当前 Profile 最近识别的店铺 ID，服务端只分配该店铺任务。
