...
已登录 Chrome 只负责官方文件导出。本机脚本负责流式解析、表头映射、个人信息剔除、脱敏检查和分块；Pages Functions 负责真实组织授权、D1 幂等写入、批次审计和对既有领域表的投影。只有允许的提供商字段进入 `erp_source_records.payload`，个人信息既不进入请求，也不泄漏进领域模型。

现有销售导入器同时识别快麦当前销售主题明细表头，在浏览器端推导净销量、净销售额、净成本和净毛利，只把 `69码 × 创建日 × 平台` 聚合行写入 `product_sales_daily`。
...
6. 增加当前库存快照：固定「库存状态」页面导出、手动触发、每日 `05:00` 计划、本机完整性校验、
   `inventory_snapshot` ingest 与 `goods_flow_inventory_daily` 投影。
7. 按相同框架继续增加采购、售后和基础资料映射。
