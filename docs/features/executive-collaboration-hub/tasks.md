...
- [x] 接入四个 App 草稿适配器和四个真实 UI 入口
  - 依赖：协同工作台和真实 App 代码。
  - 文件：四个适配器、统一按钮、产品/供应链/数据中心页面和测试。
  - 输入：App 业务记录。
  - 输出：安全、可确认的标准协同草稿。
  - 失败测试：`node --test react-tests/collaboration-adapters.test.mjs`。
  - 实现步骤：纯函数适配器后接产品、供应链、数据中心和品牌内容行操作。
  - 验证：适配器和 UI 测试通过，草稿不含敏感字段。
  - 提交：`feat(collaboration): connect business app drafts`。
  - 实际结果：2026-07-18 产品任务、供应链质量、数据质量和品牌内容数据问题均接入统一“发起协同”按钮；适配器只选择必要事实，不复制整包状态或凭据字段。适配器 3 项测试、相关 ESLint 和生产构建通过。

- [x] 把公司首页改为老板行动台并升级 App 中心
  - 依赖：协同事项和适配器。
  ...
  - 实际结果：2026-07-18 公司首页首屏改为待拍板、需要协调和重要变化，普通执行任务不进入老板队列；个人钉钉待办和战略地图保留在下方。App 中心新增未关闭协同与高影响阻塞健康度并可进入统一工作台。行动台/UI 测试 7 项、相关 ESLint 和生产构建通过。
