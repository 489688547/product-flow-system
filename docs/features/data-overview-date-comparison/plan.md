...
**Architecture:** `src/ui` 管理未提交的日期草稿，`src/domain` 计算数据中心日期范围、环比和最新日完整性，`src/state` 协调查询与显式修复请求，`src/features` 只负责组合和展示。复用现有销售事实、数据口径、快麦适配器与 D1，不新增数据库或环境变量。
...
- 不新增依赖、数据库表、变量或 Secret；自动修复通过新的平台 v1 API 复用既有快麦与 D1 边界。
...
- `src/domain/dataCenter.js`：新增 `detectLatestSalesAnomaly(latestDailyFacts)` 纯函数和 25% 规则测试。
- `functions/api/data-center/sales.js`：返回独立于所选范围的最新 8 日 GMV/销量摘要。
- `functions/api/platform/v1/data-services/sales-repair.js`：鉴权、日期校验、服务端复核、幂等调度和稳定错误契约。
- `functions/api/platform/v1/data-services/_shared/salesRepair.js`：完整分页、富退款保护、按日替换、任务状态和安全审计。
- `src/state/dataCenterApi.js` 与 `DataCenterAppPage.jsx`：显式自动触发、运行态展示和完成后刷新。
- `DataGovernanceWorkspaces.jsx`：同步记录展示自动补拉与人工文件重导结果。
