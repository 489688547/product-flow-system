...
- [ ] 任务 6：实现采购、付款、生产与收货闭环
  ...
  - 验证：权限、失败、超时、幂等和兼容测试。
  - 当前进度：已完成独立采购/付款消费视图、分页、版本化命令客户端以及 `expectedVersion + Idempotency-Key` 边界；`DEV-000006` 未交付前，批次节点与外部动作保持禁用，不标记任务完成。
...
- [ ] 任务 8：实现库存、盘点、BOM 与清仓
  ...
  - 验证：聚焦测试与真实只读库存。
  - 当前进度：已完成多仓消费视图、理论/实盘 5% 规则、组合物料归属与成本、清仓候选和缺口状态；当前/历史库存的服务端过滤和质量信息等待 `DEV-000005`。
