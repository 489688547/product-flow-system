...
供应链 feature 不直接调用快麦、钉钉、抖音或 D1。`DEV-000005` 交付共享读取契约；本计划只在 `DEV-000004` 的认领范围内实现 UI、纯领域规则、消费适配器和消费测试。需要新增的供应链工作流写 API 先形成契约需求并交给平台 owner，不在 feature 内复建整状态存储。
...
- 供应链工作流持久化契约交付前，涉及写入的新功能保持明确禁用或使用已有已验证动作，不保存到浏览器假数据。
...
每项在 `tasks.md` 记录 RED、GREEN、验证和提交证据。共享 API 未就绪时先完成契约测试与页面缺口状态，待 `DEV-000005` 合入后完成真实数据验收。
