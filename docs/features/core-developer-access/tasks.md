...
- [x] 受控签发与仓库外文件
  ...
  - 2026-08-07：签发器、0600 原子文件、哈希存储、无秘密审计、重复/身份/仓库路径
    拒绝及运行时镜像入口已实现；等待合并部署后在 ECS 快照并签发两份真实文件。
  - 2026-08-08：生产控制库备份 `quick_check=ok` 后，为张嘉豪、温雅芳分别签发独立
    `core_developer` Token；ECS 和桌面交付文件均为 0600，控制台仅传输一次性公钥
    加密后的密文，未输出明文 Token。签发指纹分别为 `04af2a20d3e6`、`f799b7e3eaa1`。
...
- [ ] README、门禁与真实验收
  ...
  - 2026-08-08：README 已补齐 Fork、upstream/dev、固定文件路径、核心/沙箱边界；
    README 合同测试与无个人文件的全新 HOME 沙箱启动已通过；张嘉豪、温雅芳两份
    文件也分别完成本地在线启动、ECS 身份解析和 session 认证。待 PR CI 与固定测试站
    完成最终验收。
