...
LaunchAgent 启动并守护本机 loopback 执行器。执行器向 jobs API 上报心跳，05:00 后创建昨天任务；扩展通过 `chrome.alarms` 最多每分钟向本机领取任务。adapter 在当前 Chrome 打开固定页面并触发官方导出，扩展通过 Downloads API 将完成结果回传本机，通用下载器确认文件稳定后交给 provider ingest。每个阶段回写 D1；只有 provider ingest 完整成功才完成任务和推进游标。
