...
- [x] 经营页面逻辑校准（2026-07-22）
  - 范围：只改电商店铺运营 App 的读取、判断和交互；数据采集由数据中心负责。
  - 输入：数据口径结果、销售事实、商品主数据、数据接入店铺目录和组织通讯录。
  - 输出：角色化驾驶舱、显式日期查询、真实主数据选择、自动证据快照、货流库存摘要、店铺/投放缺失提示。
  - 验证：定向测试完成 RED→GREEN；完整 Definition of Done 通过；1280px 与 390px 本地生产构建预览通过，390px 页面宽度与视口同为 390px；DingTalk WebView 和线上部署未执行。
