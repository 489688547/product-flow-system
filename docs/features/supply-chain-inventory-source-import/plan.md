...
提供可重复运行的钉钉库存导入工具，并在开发测试页正确展示成品、原辅料和异常库存真实数据。

## 架构方案

钉钉读取只发生在命令行集成适配器；纯解析函数将 CSV 转成供应链领域记录；脚本通过现有本地 `/api/supply-chain` 边界合并状态。前端不直接调用钉钉，未来自动同步可复用解析契约并迁入服务端任务。

## 文件职责

- `scripts/lib/dingtalkSupplyInventory.mjs`：CSV 与四类表结构的纯解析。
