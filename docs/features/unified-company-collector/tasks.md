...
- [x] 收紧统一运行时故障边界
  - 依赖：本地快麦扫描已并入。
  - 输入：独立审查发现的正式地址、跨进程互斥、凭据隔离、重试、关闭等待和代码指纹问题。
  - 输出：所有浏览器模式及回滚安装固定 ECS、可回收的共享文件锁、按操作读取 ERP token、可重试文件留队、同时启动的两个独立周期及覆盖统一入口的代码指纹。
  - 验证：跨进程争锁与陈旧锁回收、关闭 drain、上传恢复、凭据延迟读取、慢网页任务不阻塞首次扫描、正式地址和代码指纹聚焦测试全绿。
...
- [x] 写回规则并完成真实切换
  ...
  - 真实验收：PR #249 合入 `dev@5c2865de` 并通过固定测试站与独立 ECS 测试 API 冒烟；PR #254 发布 `main@d0fb8422`，正式域名精确提交、匿名会话及 Aliyun/DingTalk readiness 通过。
  - 公司 Mac：仅 `com.company.web-data-collector` 运行，PID `88642`，监听 `127.0.0.1:17653`，Ego 模式，代码指纹 `1a740319c01f`；旧快麦 LaunchAgent 已停用并保留回滚 plist，启动空扫描已更新本地扫描状态。
  - 研发待办：`DEV-000017` 已完成，版本 9，完成时间 `2026-08-08T07:30:51.874Z`。
