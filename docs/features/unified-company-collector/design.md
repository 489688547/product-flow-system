...
2. 本地文件周期：启动时及每 900 秒调用 `scanWaitingDirectory()`，使用已有 ERP token 向 ECS ingest 上传。
3. 本地归档串行器：浏览器下载交付和待导入目录扫描通过同一异步互斥边界更新归档文件、manifest 和扫描状态。

本地文件周期失败只产生安全日志和下一轮重试，不抛出到顶层结束服务。网页周期使用单一防重入计时器，本地扫描由周期对象独立节流；进程停止时清理计时器并关闭 loopback。
