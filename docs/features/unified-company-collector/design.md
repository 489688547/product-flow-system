...
3. 本地归档串行器：浏览器下载交付和待导入目录扫描通过同一异步队列及归档根目录 SQLite 即时事务锁更新归档文件、manifest 和扫描状态，并写 owner/PID 哨兵兼容旧 CLI。旧 CLI 与新服务不能跨进程并发写；进程消失时操作系统释放事务锁，下一次操作核对 PID 后回收陈旧哨兵。
