...
3. 本地归档串行器：浏览器下载交付和待导入目录扫描通过同一异步队列及归档根目录 SQLite 即时事务锁更新归档文件、manifest 和扫描状态，并写 owner/PID 哨兵兼容旧 CLI。旧 CLI 与新服务不能跨进程并发写；进程消失时操作系统释放事务锁，下一次操作只在 PID 已确认不存在时回收陈旧哨兵。无法确认 owner/PID 的旧锁保持阻塞并要求人工核对，不能按文件年龄推断进程已经退出。

本地文件周期失败只产生安全日志和下一轮重试，不抛出到顶层结束服务。网络错误与 5xx 保留源文件等待重试，解析或口径错误才进入失败目录。网页周期与本地扫描在服务启动时同时触发，并分别防重入和计时，任一慢任务不阻塞另一周期；进程停止时先停止新周期，再等待在途周期和归档队列排空后关闭。
