...
- `npm start` 是唯一支持的完整本地业务运行时。8127 的 Vite 提供单一浏览器入口和热更新，`/api` 代理到内部 8132 Wrangler；线上模式把 Pages Functions 构建为 Worker 后使用 `wrangler dev --remote` 在 Cloudflare 网络执行，沙箱模式才使用本地 Pages Dev。
- 远程开发 Worker 原生绑定生产 D1，并从被忽略的 `.env` 加载个人令牌；原始令牌不得进入浏览器、仓库、响应或日志。
- 启动器每次生成一次性传输密钥，Vite 仅在服务端代理 Header 中发送，远程开发 Worker 同时持有该密钥。API 中间件只在回环主机，或密钥匹配的远程开发请求中，结合 `LOCAL_ONLINE_ACCOUNT_MODE=1` 校验令牌哈希、read/write 能力及 `product_flow_org_members` 中的 active executive 稳定身份。已部署环境不配置该密钥。
...
收益是本地能够在部署前验证完整真实链路，数据、账号和外部动作不再因环境分叉。风险是未部署代码可以直接改变生产数据或外部平台；通过真实账号校验、一次性传输密钥、服务端令牌、全局风险提示、业务原有版本与幂等规则控制。外部动作通常不可自动回滚，因此自动化测试不得创建无业务意义的真实对象。
