...
| 页面结构变化 | `DOUYIN_PAGE_SCHEMA_CHANGED`、`DOUYIN_REPORT_SCHEMA_CHANGED`、`KUAIMAI_ORDER_PAGE_SCHEMA_CHANGED`、`KUAIMAI_EXPORT_REQUIRED_COLUMNS_MISSING`、`KUAIMAI_API_TIME_TYPE_INVALID`、`KUAIMAI_API_BUSINESS_DATE_INVALID`、`KUAIMAI_API_MALFORMED`、`KUAIMAI_SALES_DATE_PICK_FAILED`、`DOUYIN_EXTRACT_RANGE_TOO_LONG` | 否，重试必然再失败 |
