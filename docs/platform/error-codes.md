...
- `ERP_COLLECTION_ARCHIVE_PROCESSING_TIMEOUT`：归档处理超过 24 小时，服务端已将其转为失败；原始文件、关联批次和上一可信事实保持不变。
- `ERP_COLLECTION_ARCHIVE_NOT_FOUND`：要记录入库决策的归档 ID 不存在。
- `ERP_COLLECTION_ARCHIVE_DECISION_INVALID` / `ERP_COLLECTION_ARCHIVE_REASON_INVALID`：入库决策、预期版本或受控原因不合法。
- `ERP_COLLECTION_ARCHIVE_STATE_CONFLICT` / `ERP_COLLECTION_ARCHIVE_VERSION_CONFLICT`：当前运行状态不能记录跳过决策，或读取版本已经过期。
- `ERP_COLLECTION_BATCH_PARTIAL`：快麦销售明细原始记录已上传，但最终批次未通过完整性校验；网页采集任务必须失败或重试，不能显示“同步成功”。
- `ERP_COLLECTION_SALES_FACT_INVALID` / `ERP_COLLECTION_SALES_FACTS_TOO_LARGE`：本机聚合后的销售事实缺少有效 69 码/日期，或单次超过 5,000 条标准事实。
...
- `WEB_COLLECTION_TRIGGER_INVALID`：用户触发请求不是已登记的快麦订单、销售明细或商品快照入口，或业务日期无效。
...
- `DINGTALK_TODO_REPLACEMENT_RETIRE_FAILED`：新版工作待办已创建，但旧卡片未能退出未完成列表；内部 Provider 适配错误，HTTP 502，可重试且不得保存新版绑定。
...
- `DATA_SERVICE_DATE_RANGE_INVALID`：销售日需求日期或筛选无效。
- `DATA_TASKS_STORAGE_UNAVAILABLE` / `DATA_TASKS_QUERY_FAILED`：统一数据任务控制面不可用或查询失败。
- `GOODS_FLOW_INVENTORY_QUERY_INVALID` / `GOODS_FLOW_QUERY_INVALID`：库存或货流集合筛选无效。
- `SUPPLY_WORKFLOW_VIEW_DENIED` / `SUPPLY_WORKFLOW_ACTION_DENIED` / `SUPPLY_WORKFLOW_WRITE_DENIED`：当前身份无读取、责任部门动作或写权限。
- `SUPPLY_WORKFLOW_INPUT_INVALID` / `SUPPLY_WORKFLOW_SERVER_FIELD_DENIED` / `SUPPLY_WORKFLOW_SENSITIVE_FIELD_DENIED`：输入无效、提交了服务端字段或试图保存秘密。
- `SUPPLY_WORKFLOW_IDEMPOTENCY_KEY_REQUIRED` / `SUPPLY_WORKFLOW_IDEMPOTENCY_CONFLICT`：幂等键缺失或已用于其他操作。
- `SUPPLY_WORKFLOW_VERSION_CONFLICT`：实体版本已变化，HTTP 409。
- `SUPPLY_WORKFLOW_TRANSITION_INVALID`：当前状态不允许该动作。
- `SUPPLY_WORKFLOW_STORAGE_UNAVAILABLE`：业务 D1 或新增工作流表未就绪。
...
- `GOODS_FLOW_INVENTORY_SNAPSHOT_INVALID`：完整库存快照为空、混合日期、缺少稳定 SKU/仓库，或包含重复 `SKU × 仓库`，HTTP 400；不得替换上一可信快照。
...
- `CORE_DEVELOPER_CAPABILITY_REQUIRED`：个人令牌没有显式核心开发能力，HTTP 403。
- `CORE_DEVELOPER_STORAGE_UNAVAILABLE` / `CORE_DEVELOPER_AUTH_UNAVAILABLE`：ECS 控制库或核心开发认证不可用，HTTP 503。
- `CORE_DEVELOPER_PROXY_HOST_FORBIDDEN` / `CORE_DEVELOPER_PROXY_ORIGIN_FORBIDDEN` / `CORE_DEVELOPER_PROXY_ORIGIN_REQUIRED`：本地代理拒绝非固定回环 Host、外部 Origin 或缺少 Origin 的写请求，HTTP 403。
- `DEVELOPER_ACCESS_INVALID` / `DEVELOPER_ACCESS_AMBIGUOUS` / `DEVELOPER_ACCESS_MODE_INVALID` / `DEVELOPER_ACCESS_OWNER_INVALID`：个人文件内容、多文件、0600 权限修复或所有者无效，启动前失败。
- `CORE_DEVELOPER_IDENTITY_INVALID` / `CORE_DEVELOPER_TOKEN_EXISTS` / `CORE_DEVELOPER_OUTPUT_FORBIDDEN`：签发目标身份不稳定、已有有效 Token 或输出位于 Git 仓库，签发失败且不输出明文。
...
| 分类 | 错误码 | 可自助重试 |
|---|---|---|
| 页面交互未生效 | `DOUYIN_DATE_RANGE_NOT_APPLIED`、`DOUYIN_DATE_CONTROL_MISSING`、`KUAIMAI_TIME_RANGE_NOT_APPLIED`、`KUAIMAI_SALES_EXPORT_CONFIRM_MISSING`、`DOUYIN_OFFICIAL_REPORT_BUTTON_MISSING`、`KUAIMAI_API_REQUEST_FAILED`、`KUAIMAI_API_TOTAL_MISMATCH`、`KUAIMAI_SALES_CALCULATE_TIMEOUT`、`DOUYIN_EXTRACT_TIMEOUT`、`DOUYIN_EXTRACT_TASK_FAILED`、`DOUYIN_EXTRACT_DAYS_MISSING`、`DOUYIN_EXTRACT_DATE_NOT_APPLIED` | 是 |
| 页面结构变化 | `DOUYIN_PAGE_SCHEMA_CHANGED`、`DOUYIN_REPORT_SCHEMA_CHANGED`、`KUAIMAI_ORDER_PAGE_SCHEMA_CHANGED`、`KUAIMAI_EXPORT_REQUIRED_COLUMNS_MISSING`、`KUAIMAI_API_TIME_TYPE_INVALID`、`KUAIMAI_API_BUSINESS_DATE_INVALID`、`KUAIMAI_API_MALFORMED`、`KUAIMAI_SALES_DATE_PICK_FAILED`、`DOUYIN_EXTRACT_RANGE_TOO_LONG`、`DOUYIN_EXTRACT_DATE_COLUMN_MISSING` | 否，重试必然再失败 |
...
`KUAIMAI_API_TIME_TYPE_INVALID` 在发请求前抛出，不是事后判断。快麦查询接口对无效
`timeType` 静默回落到付款时间，既不报错也不提示，2026-07-25 实测与订单创建时间口径
相差 399 单（约 6%）。这类故障在响应里没有任何征兆，只能靠请求前的白名单拦住。

快麦页面上的日期控件不接受程序化写值。销售报表的日期框是 Element UI 只读选择器
（`readOnly=true`），写 `value` 只改 DOM 不更新 Vue 模型：实测两端都写成 2026-07-26
后输入框显示正确，计算请求带的却仍是页面原范围 2026-07-22 ~ 2026-07-28。因此断言
只比对输入框的值是无效的——那正是采集器自己刚写进去的，必然通过。必须打开日期
面板点日期格，并以实际请求参数为准。

`KUAIMAI_SALES_CALCULATE_TIMEOUT` 宁可不导出也不导半成品。判定「算完了」时不能只看
「遮罩消失且表格有行」：点击「计算数据」后遮罩要过一会儿才挂上，在那之前页面上还留着
上一个任务算好的表格，行数大于 0 又没有遮罩，会被当成本轮已完成，导出的是上一天的
数据。实测目标 07-27 的导出里 07-26 占 24777 个单元格、07-27 只有 5498 个；而一批
任务里的第一个（页面无旧结果）总是成功。必须先确认本轮计算已启动——等遮罩出现，
或确认表格内容确实换过。

`KUAIMAI_SALES_CALCULATE_TIMEOUT` 宁可不导出也不导半成品。销售报表的「导出」按钮
在计算过程中始终可点，不能当完成信号；实测点完「计算数据」第 2 秒时遮罩还在、表格
还是 0 行，导出按钮却已可点。固定等待若短于计算耗时，导出的就是中间结果：2026-07-26
只落了 176 行 ¥8,498，正常约 530 行 ¥13 万，而任务状态显示成功。完成信号必须是
遮罩消失且表格出现数据行。

`KUAIMAI_API_TOTAL_MISMATCH` 表示整批作废而非部分成功。宁可判失败让人重来，也不能
入库一半——2026-07-25 至 07-27 的缺口正是「采到一部分就当成功」造成的。
