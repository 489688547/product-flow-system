...
| 分类 | 错误码 | 可自助重试 |
|---|---|---|
| 页面交互未生效 | `DOUYIN_DATE_RANGE_NOT_APPLIED`、`DOUYIN_DATE_CONTROL_MISSING`、`KUAIMAI_TIME_RANGE_NOT_APPLIED`、`KUAIMAI_SALES_EXPORT_CONFIRM_MISSING`、`DOUYIN_OFFICIAL_REPORT_BUTTON_MISSING`、`KUAIMAI_API_REQUEST_FAILED`、`KUAIMAI_API_TOTAL_MISMATCH`、`KUAIMAI_SALES_CALCULATE_TIMEOUT` | 是 |
| 页面结构变化 | `DOUYIN_PAGE_SCHEMA_CHANGED`、`DOUYIN_REPORT_SCHEMA_CHANGED`、`KUAIMAI_ORDER_PAGE_SCHEMA_CHANGED`、`KUAIMAI_EXPORT_REQUIRED_COLUMNS_MISSING`、`KUAIMAI_API_TIME_TYPE_INVALID`、`KUAIMAI_API_BUSINESS_DATE_INVALID`、`KUAIMAI_API_MALFORMED`、`KUAIMAI_SALES_DATE_PICK_FAILED` | 否，重试必然再失败 |
