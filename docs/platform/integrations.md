...
回流串行、有界查询当前用户的未完成与已完成企业工作待办，并在过渡期独立尝试读取个人待办；任一来源失败不会阻断另一来源，响应按 todoId 去重并返回授权、成功、截断覆盖和下一轮游标。2026-07-27 生产真机验证确认，个人待办创建接口返回的 `task…` ID 不进入企业待办查询结果，因此个人待办不得继续作为新产品任务的双向同步绑定。已绑定个人待办不自动删除；服务端只允许产品负责人、原创建人或执行人以 `:rN` 稳定来源创建工作待办并替换系统绑定。查询窗口内的待办一次回流，超过窗口时按游标逐轮覆盖，不承诺所有历史待办均在单次 60 秒周期内命中。浏览器保存的仅是页码和不透明 `nextToken`，后续轮转读取更后页，不保存待办正文。浏览器只为当前账号已分配的待办自动刷新；同标签页合并请求、跨标签页使用 45 秒刷新租约、首次及周期轮询增加随机抖动，来源级或顶层 429 后均退避 2 分钟，避免放大企业 40 QPS 峰值。客户端只允许本地已有可信稳定来源的 todoId，或远端当前任务稳定 sourceId 命中的卡片写回；来源为空的历史 ID、其他个人待办、Teambition 项目任务和其他应用待办不得误绑定。所有供应商错误均归一为安全错误码、提示和可重试标识，浏览器不得收到 MCP/OpenAPI 原始响应。
