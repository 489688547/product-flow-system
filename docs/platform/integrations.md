...
产品任务默认使用钉钉企业工作待办：服务端通过平台连接解析应用访问凭证，调用企业工作待办创建接口，并把返回 todoId、本地稳定 `sourceId` 与服务端确认的创建人 unionId 一起保存。创建请求同时包含正文、截止时间、优先级、unionId 执行人和详情链接；该待办必须能从企业待办未完成/已完成查询接口读回，才属于双向同步主链路。更新已绑定待办时，先写执行者完成状态，再通过工作待办更新接口写整体 `done` 和正文等字段；两步都成功才持久化最新同步快照，从而支持钉钉完成回流和系统取消完成后恢复钉钉未完成。Provider 成功后，待办同步路由必须在返回成功前基于最新共享状态原子保存真实 todoId；若版本已推进，只能重新读取并重放当前任务绑定，有限重试仍冲突则返回 `DINGTALK_TODO_BINDING_CONFLICT`，不得依赖浏览器随后保存整份状态。成功响应带回已落库任务和新状态版本，客户端据此推进共享基线。展示环境保存确定性模拟绑定并显示“展示模拟”，不得显示为真实“已同步”。
