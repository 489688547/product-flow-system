...
产品任务默认使用钉钉原生个人待办：服务端只从当前受认证会话读取加密保存的用户访问凭证，调用个人待办创建接口，并把返回 todoId、本地稳定 `sourceId` 与服务端确认的创建人 unionId 一起保存。正文、截止时间和 unionId 执行人在创建请求中发送；个人接口未提供的优先级通过已登记的钉钉待办 MCP 串行设置。创建成功后的优先级补写失败必须保留 todoId 并进入可重试状态，禁止把整次创建当作失败而生成重复待办。更新只允许原创建人执行：个人待办 MCP 先更新标题、优先级、截止时间和完成状态，应用待办接口再补正文和执行人；后半段失败返回部分成功警告并复用原 todoId 重试。
