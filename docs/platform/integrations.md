...
产品任务默认使用钉钉原生个人待办：服务端只从当前受认证会话读取加密保存的用户访问凭证，调用个人待办创建接口，并把返回 todoId、本地稳定 `sourceId` 与服务端确认的创建人 unionId 一起保存。Provider 成功后，待办同步路由必须在返回成功前基于最新共享状态原子保存真实 todoId；若版本已推进，只能重新读取并重放当前任务绑定，有限重试仍冲突则返回 `DINGTALK_TODO_BINDING_CONFLICT`，不得依赖浏览器随后保存整份状态。成功响应带回已落库任务和新状态版本，客户端据此推进共享基线。展示环境保存确定性模拟绑定并显示“展示模拟”，不得显示为真实“已同步”。正文、截止时间和 unionId 执行人在创建请求中发送；个人接口未提供的优先级通过已登记的钉钉待办 MCP 串行设置。创建成功后的优先级补写失败必须保留 todoId 并进入可重试状态，禁止把整次创建当作失败而生成重复待办。更新只允许原创建人执行：个人待办 MCP 先更新标题、优先级、截止时间和完成状态，应用待办接口再补正文和执行人；后半段失败返回部分成功警告并复用原 todoId 重试。

回流串行、有界查询当前用户的未完成与已完成个人待办，并在迁移期独立补充本应用企业工作待办；任一来源失败不会阻断另一来源，响应按 todoId 去重并返回授权、成功、截断覆盖和下一轮游标。查询窗口内的待办一次回流，超过窗口时按游标逐轮覆盖，不承诺所有历史待办均在单次 60 秒周期内命中。浏览器保存的仅是页码和不透明 `nextToken`，后续轮转读取更后页，不保存待办正文。浏览器只为当前账号已分配的待办自动刷新；同标签页合并请求、跨标签页使用 45 秒刷新租约、首次及周期轮询增加随机抖动，来源级或顶层 429 后均退避 2 分钟，避免放大企业 40 QPS 峰值。客户端只允许本地已有可信稳定来源的 todoId，或远端当前任务稳定 sourceId 命中的卡片写回；来源为空的历史 ID、其他个人待办、Teambition 项目任务和其他应用待办不得误绑定。旧版个人待办若缺少创建人，只允许服务端确认的产品负责人或原执行人创建带 `:rN` 来源的受控替代待办，其他用户仍关闭失败。个人凭证缺失或过期会明确提示重新钉钉登录，同时保留可用的迁移期工作待办回流，不得静默降级创建新的企业工作待办。所有供应商错误均归一为安全错误码、提示和可重试标识，浏览器不得收到 MCP/OpenAPI 原始响应。
