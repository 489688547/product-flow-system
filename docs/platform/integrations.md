...
回流使用应用访问凭证调用当前用户的 `/v1.0/todo/users/{unionId}/organizations/tasks/query`，请求固定 `needPersonalTodo=true`，串行、有界读取未完成与已完成状态；该查询同时覆盖原生个人待办和当前企业工作待办，历史卡片迁移不再依赖另一套 MCP 列表。响应按 todoId 去重并返回成功、截断覆盖和下一轮游标。2026-07-28 生产真机验证确认，个人待办创建接口返回的 `task…` ID 不进入 `/org/tasks/query`，但会进入带 `needPersonalTodo=true` 的 `/organizations/tasks/query`；不得再用前一接口“查不到”判定创建失败。查询窗口内的待办一次回流，超过窗口时按不透明 `nextToken` 逐轮覆盖，不承诺所有历史待办均在单次 60 秒周期内命中。浏览器只为当前账号已分配的待办自动刷新；同标签页合并请求、跨标签页使用 45 秒刷新租约、首次及周期轮询增加随机抖动，顶层 429 后退避 2 分钟，避免放大企业 40 QPS 峰值。客户端只允许本地已保存的可信个人 taskId 写回当前产品任务；来源为空的历史 ID、其他个人待办、Teambition 项目任务和其他应用待办不得误绑定。所有供应商错误均归一为安全错误码、提示和可重试标识，浏览器不得收到 OpenAPI 原始响应。
