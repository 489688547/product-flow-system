...
回流以当前用户授权的个人待办查询为主，串行、有界读取未完成与已完成状态；企业工作待办查询仅保留给历史卡片迁移。任一来源失败不会阻断另一来源，响应按 todoId 去重并返回授权、成功、截断覆盖和下一轮游标。2026-07-27 生产真机验证确认，个人待办创建接口返回的 `task…` ID 不进入企业工作待办查询结果，因此个人 taskId 必须通过个人待办查询通道核验，不能再用企业待办查询“查不到”判定创建失败。查询窗口内的待办一次回流，超过窗口时按游标逐轮覆盖，不承诺所有历史待办均在单次 60 秒周期内命中。浏览器保存的仅是页码和不透明 `nextToken`，后续轮转读取更后页，不保存待办正文。浏览器只为当前账号已分配的待办自动刷新；同标签页合并请求、跨标签页使用 45 秒刷新租约、首次及周期轮询增加随机抖动，来源级或顶层 429 后均退避 2 分钟，避免放大企业 40 QPS 峰值。客户端只允许本地已保存的可信个人 taskId 写回当前产品任务；来源为空的历史 ID、其他个人待办、Teambition 项目任务和其他应用待办不得误绑定。所有供应商错误均归一为安全错误码、提示和可重试标识，浏览器不得收到 MCP/OpenAPI 原始响应。
