...
当前只接受 `platformId=lingsuan-ai-gateway`。`purpose` 必填且不超过 200 字；确认语必须完全一致。每个连接 15 分钟最多成功查看 5 次。
...
响应固定使用 `Cache-Control: private, no-store` 和 `Pragma: no-cache`。浏览器只能保存在 React 内存中，并在 5 分钟、设置收起、页面隐藏、保存、停用或卸载时清除。
...
- 所有响应和审计不包含测试凭据。
- reveal 覆盖最高权限、总经办普通成员、旧会话、确认语、用途、环境回退、no-store、5 次限流和无秘密审计。
