...
```json
{
  "metricCodes": ["sales.net_sales", "sales.gross_profit"],
  "from": "2026-07-01",
  "to": "2026-07-19",
  "targetVersions": {},
  "mode": "ensure_current",
  "confirmed": false,
  "idempotencyKey": "客户端值仅用于兼容，服务端不会信任"
}
```

`metricCodes` 去重后至少 1 项，连同公式依赖最多 11 项；日期使用上海时区的真实自然日，首尾均包含，最多 370 天；依赖深度最多 8 层。未指定目标版本时，按查询截止日选择当时已生效的最新版本，不使用未来版本。

`ensure_current` 允许任一数据口径查看者为当前范围补齐幂等派生结果。`explicit_recalculation` 必须提交 `confirmed: true`，并且仅允许总经办，或同时拥有所有目标口径责任部门权限的非只读身份。`targetVersions` 只能引用本次指标及其依赖的已存在版本。

服务端用排序后的指标、范围、实际版本和销售事实水位生成 SHA-256 幂等摘要；客户端 `idempotencyKey` 不能决定去重边界。首次请求建立 `pending` run，通过 Pages `waitUntil` 在响应后执行计算，并立即返回 HTTP 202。重复请求返回同一 run，不建立新批次，也不重复调度。公开 run 只包含 ID、状态、进度、错误码、范围和时间，不返回服务端摘要或发起人内部字段。

### GET 读取计算结果

支持 `metricCodes`（逗号分隔）、`from`、`to`、`dimensions`（URL 编码 JSON）和可选 `runId`。不带 `runId` 时只读 `is_current = 1`，且 `from/to` 精确匹配结果批次范围；带 `runId` 仅用于该批次的轮询和审计，不改变当前结果。

```json
{
  "synced": true,
  "run": { "id": "metric_run_example", "status": "succeeded", "progress": 100, "errorCode": "" },
  "results": [{
...
无结果返回空数组和 `missing.reasonCode`，绝不补 0。轮询中的批次返回 `CALCULATION_PENDING`，失败批次返回 run 的安全错误码；普通缺失返回 `RESULT_NOT_AVAILABLE`。GET 不触发计算、不写库。
...
| `DATA_STANDARD_QUERY_RANGE_INVALID` | 400 | 否 | 指标数、依赖深度、日期或范围超过计算限制 |
| `DATA_STANDARD_CALCULATION_FAILED` | 500 | 是 | 后台计算失败；原当前结果保持有效 |
...
- 计算请求的幂等摘要由服务端根据指标、范围、实际版本和事实水位计算。相同输入命中同一批次；事实水位或版本改变后建立新批次。
- 计算结果先以 `is_current = 0` 写入；同一 D1 batch 中将旧范围结果撤为历史、把新批次切为 current，并将 run 标记为 succeeded。任何一步失败时整批不生效，旧 current 继续可读。

## 超时、重试与可观测性

路由只访问 D1，不调用钉钉、快麦或其他外部平台。计算由 `waitUntil` 承接，调用方用 `runId` 轮询；前端轮询必须有次数上限并支持取消。后台失败将 run 标记为 failed，保留旧 current；调用方可在事实或版本修正后重新发起。服务端不自动重放失败批次。只有存储不可用、后台计算失败和未预期服务错误标记为可重试。
...
- `tests/data-standards-calculation.test.mjs`：安全编译、历史版本选择、空值、未覆盖和预览一致性。
- `tests/data-standards-results-api.test.mjs`：异步调度、服务端幂等、权限、限制、失败隔离和结果读取。
