...
- 公司 Mac 只运行 `com.company.web-data-collector` 一个常驻采集服务：网页任务保持 60 秒周期，本地快麦待导入目录在启动时及每 900 秒独立扫描一次；两条路径同时受进程内队列和归档根目录 `.collector.lock` 保护。旧 `kuaimai-erp-collector` CLI 只保留人工与回滚命令，并复用同一文件锁，其 LaunchAgent 不再正式启用。正式 base URL 固定为 `https://deshan-tiyes.cn`，不因浏览器模式变化而放宽；日志固定写入 `~/Library/Logs/product-flow/`。
