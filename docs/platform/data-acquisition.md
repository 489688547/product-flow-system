...
- `claimed`、`opening`、`exporting`、`downloading`、`validating` 或 `ingesting` 阶段的设备租约过期表示本机执行中断；控制面允许其他轮询重新领取同一任务并增加 attempt，最多 3 次。不能只恢复 `claimed`，否则进程在后续阶段退出会让任务永久显示“同步中”。
...
- Provider adapter 必须覆盖同一官方操作在真实页面出现的已验证弹窗容器变体。快麦销售主题导出当前同时支持普通 `el-dialog` 与确认型 `el-message-box`；只在容器内按精确按钮文字执行，不能退化为全页模糊点击。
...
- ERP 批次的 `partial` 表示允许的终态（例如部分来源行因映射问题未投影），不是仍在处理；最终分块到达 `partial` 或 `completed` 时原始归档都标记为 `processed`，仅 `pending` 保持 `processing`。数据质量问题通过 issue 记录闭环，不能让已结束归档永久显示“入库中”。
