...
- 公司 Mac 只运行 `com.company.web-data-collector` 一个常驻采集服务：网页任务保持 60 秒周期，本地快麦待导入目录在启动时及每 900 秒扫描一次；两条路径共用串行归档边界。旧 `kuaimai-erp-collector` CLI 只保留人工与回滚命令，其 LaunchAgent 不再正式启用。正式 base URL 固定为 `https://deshan-tiyes.cn`，日志固定写入 `~/Library/Logs/product-flow/`。
