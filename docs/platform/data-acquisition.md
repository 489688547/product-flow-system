...
- 公司 Mac 只运行 `com.company.web-data-collector` 一个常驻采集服务：网页任务保持 60 秒周期，本地快麦待导入目录在启动时立即、之后每 900 秒独立扫描一次，任一慢任务不能推迟另一周期启动；两条路径同时受进程内队列和归档根目录 `.collector.lock` 保护。文件锁记录 owner 与 PID，进程已不存在时可安全回收，仍存活时不得抢占。旧 `kuaimai-erp-collector` CLI 只保留人工与回滚命令，并复用同一文件锁，其 LaunchAgent 不再正式启用。新旧安装命令的正式 base URL 都固定为 `https://deshan-tiyes.cn`，不因浏览器模式或回滚而放宽；日志固定写入 `~/Library/Logs/product-flow/`。
