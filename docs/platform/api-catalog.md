...
| `/api/dingtalk/todo/sync` | 创建或更新产品任务的钉钉企业工作待办并保存双向同步绑定 | 需要非只读公司会话；服务端校验稳定任务来源及创建人；应用凭证创建可由企业待办列表回读的工作待办；已绑定个人待办仅允许可信负责人迁移为 `:rN` 工作待办；Provider 成功后以最新共享状态原子保存 todoId，冲突时仅重放当前任务绑定；成功返回已落库任务与新 `updatedAt`；展示环境仅返回并保存模拟结果 |
...
展示数据库更新采用创建任务、读取进度和续跑单步三个接口。阶段固定为预检、清理、复制、转换、重算、校验和启用；同一时间最多一个活动任务，每步只处理一个有界批次并持有短租约。浏览器轮询并续跑，关闭后可按任务 ID 恢复；相同幂等键重复请求返回同一任务。D1 网络、连接、超时等瞬时存储错误在当前游标原地重试，连续 3 次失败后才终止任务，稳定的结构、数据和权限错误不自动重试。只有全部校验通过才推进环境版本并恢复可用，失败记录安全结果码且不提供半成品数据。
