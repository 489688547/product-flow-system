...
| `/api/dingtalk/todo/sync` | 创建或更新产品任务的钉钉个人待办并保存双向同步绑定 | 需要非只读公司会话和当前用户 OAuth；服务端校验稳定任务来源及创建人；Provider 成功后以最新共享状态原子保存 todoId，冲突时仅重放当前任务绑定；成功返回已落库任务与新 `updatedAt`；展示环境仅返回并保存模拟结果 |
