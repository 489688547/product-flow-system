...
Cloudflare 的 Git 仓库构建环境当前使用 Wrangler 3.x 打包 Pages Functions，而本地发布使用当前 Wrangler。修改 `functions/` 后必须同时运行兼容性测试，并确认 Git 自动部署与本地 CLI 打包都成功；不要只以本地新版本 Wrangler 的结果判断可发布。
